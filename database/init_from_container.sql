...
DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'api_key_type') THEN
...
DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'chat_context_level') THEN
...
DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'credential_request_status') THEN
...
DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'credential_share_mode') THEN
...
DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'dashboard_view_type') THEN
...
DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'policy_type') THEN
...
DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'request_category') THEN
...
DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'ticket_base_status') THEN
...
CREATE OR REPLACE FUNCTION public.acknowledge_dns_alert(p_alert_id uuid, p_user_id uuid, p_is_expected boolean DEFAULT false, p_notes text DEFAULT NULL::text) RETURNS boolean
    LANGUAGE plpgsql
    AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION public.acknowledge_kb_article(p_article_id uuid, p_user_id uuid DEFAULT NULL::uuid, p_contact_id uuid DEFAULT NULL::uuid, p_ip_address character varying DEFAULT NULL::character varying, p_user_agent text DEFAULT NULL::text) RETURNS boolean
    LANGUAGE plpgsql
    AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION public.add_contribution_columns(p_table_name text) RETURNS void
    LANGUAGE plpgsql
    AS $$
BEGIN
    -- Soft delete columns
...
CREATE OR REPLACE FUNCTION public.ai_get_ticket(p_session_id uuid, p_ticket_id uuid) RETURNS TABLE(id uuid, ticket_number text, subject character varying, description text, status_name character varying, priority character varying, category_name character varying, created_at timestamp with time zone, updated_at timestamp with time zone, can_view_details boolean)
...
CREATE OR REPLACE FUNCTION public.ai_search_kb(p_session_id uuid, p_query text, p_limit integer DEFAULT 5) RETURNS TABLE(id uuid, title character varying, excerpt text, category_name character varying, relevance_score double precision)
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
DECLARE
    v_session RECORD;
...
BEGIN
    ...
    -- Search with visibility filter (table: kb_articles)
    RETURN QUERY
    SELECT
        kb.id,
        kb.title,
...
CREATE OR REPLACE FUNCTION public.approve_category_request(p_request_id uuid, p_reviewed_by uuid, p_final_name character varying DEFAULT NULL::character varying, p_notes text DEFAULT NULL::text) RETURNS uuid
    LANGUAGE plpgsql
...
--
-- Name: approve_credential_access(uuid, uuid, integer); Type: FUNCTION; Schema: public; Owner: -
--

CREATE OR REPLACE FUNCTION public.approve_credential_access(p_request_id uuid, p_approved_by uuid, p_duration_hours integer DEFAULT NULL::integer) RETURNS boolean
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_request credential_access_requests%ROWTYPE;
    v_policy credential_sharing_policies%ROWTYPE;
    v_duration INT;
BEGIN
    SELECT * INTO v_request FROM credential_access_requests WHERE id = p_request_id;
    IF NOT FOUND OR v_request.status != 'pending' THEN
        RETURN false;
    END IF;
    
    SELECT * INTO v_policy FROM credential_sharing_policies WHERE id = v_request.policy_id;
    v_duration := COALESCE(p_duration_hours, v_policy.access_duration_hours, 8);
    ...
    SET status = 'approved',
        ...
        access_expires_at = NOW() + (v_duration || ' hours')::INTERVAL,
...
CREATE OR REPLACE FUNCTION public.assign_onboarding_policies(p_contact_id uuid) RETURNS integer
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_contact RECORD;
...
CREATE OR REPLACE FUNCTION public.auto_assign_ticket(p_ticket_id uuid, p_team_id uuid) RETURNS uuid
    LANGUAGE plpgsql
...
CREATE OR REPLACE FUNCTION public.calculate_next_occurrence(p_schedule_type character varying, p_schedule_config jsonb, p_from_date timestamp with time zone DEFAULT now()) RETURNS timestamp with time zone
    LANGUAGE plpgsql
    AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION public.can_ai_access_resource(p_session_id uuid, p_resource_type character varying, p_resource_id uuid DEFAULT NULL::uuid) RETURNS boolean
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
DECLARE
    v_session RECORD;
...
CREATE OR REPLACE FUNCTION public.can_delegate_action(actor_uuid uuid, target_uuid uuid, action_name character varying, org_uuid uuid) RETURNS boolean
...
--
-- Name: can_provider_access_credential(uuid, uuid); Type: FUNCTION; Schema: public; Owner: -
--

CREATE OR REPLACE FUNCTION public.can_provider_access_credential(p_credential_id uuid, p_api_key_id uuid) RETURNS TABLE(can_access boolean, access_mode public.credential_share_mode, request_id uuid, expires_at timestamp with time zone, reveals_remaining integer, requires_checkin boolean, checkin_due_at timestamp with time zone, message text)
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_policy credential_sharing_policies%ROWTYPE;
...
BEGIN
    -- Get policy
    SELECT * INTO v_policy 
    FROM credential_sharing_policies 
    WHERE credential_id = p_credential_id 
      AND (provider_api_key_id = p_api_key_id OR provider_api_key_id IS NULL)
    ORDER BY provider_api_key_id NULLS LAST
    LIMIT 1;
    
    IF NOT FOUND OR v_policy.share_mode = 'never' THEN
        RETURN QUERY SELECT 
            false, 'never'::credential_share_mode, NULL::UUID, NULL::TIMESTAMP WITH TIME ZONE,
            NULL::INT, NULL::BOOLEAN, NULL::TIMESTAMP WITH TIME ZONE,
            'Credential sharing not enabled'::TEXT;
        RETURN;
    END IF;
    
    -- Get active access request
    SELECT * INTO v_request
    FROM credential_access_requests
    WHERE credential_id = p_credential_id
      AND provider_api_key_id = p_api_key_id
      AND status = 'approved'
    ...
    IF NOT FOUND THEN
        RETURN QUERY SELECT 
            false, v_policy.share_mode, NULL::UUID, NULL::TIMESTAMP WITH TIME ZONE,
            NULL::INT, NULL::BOOLEAN, NULL::TIMESTAMP WITH TIME ZONE,
            'No active access grant. Request access first.'::TEXT;
        RETURN;
    END IF;
    
    -- Check if check-in is overdue
    IF v_policy.check_in_required AND v_request.last_check_in_at IS NOT NULL THEN
        IF v_request.last_check_in_at + (v_policy.check_in_interval_hours || ' hours')::INTERVAL < NOW() THEN
            -- Revoke access due to missed check-in
            UPDATE credential_access_requests SET status = 'revoked' WHERE id = v_request.id;
            
            RETURN QUERY SELECT 
                false, v_policy.share_mode, v_request.id, v_request.access_expires_at,
                NULL::INT, true, NULL::TIMESTAMP WITH TIME ZONE,
                'Access revoked due to missed check-in'::TEXT;
            RETURN;
        END IF;
    END IF;
    
    -- Check reveal limits
    IF v_policy.max_reveals_per_session IS NOT NULL AND 
       v_request.reveals_count >= v_policy.max_reveals_per_session THEN
        RETURN QUERY SELECT 
            false, v_policy.share_mode, v_request.id, v_request.access_expires_at,
            0, v_policy.check_in_required, 
            v_request.last_check_in_at + (v_policy.check_in_interval_hours || ' hours')::INTERVAL,
            'Maximum reveals reached for this session'::TEXT;
        RETURN;
    END IF;
    ...
    RETURN QUERY SELECT 
        ...
        v_policy.share_mode, 
        v_request.id, 
        v_request.access_expires_at,
        COALESCE(v_policy.max_reveals_per_session, 999) - v_request.reveals_count,
        v_policy.check_in_required,
        CASE WHEN v_policy.check_in_required 
             THEN v_request.last_check_in_at + (v_policy.check_in_interval_hours || ' hours')::INTERVAL
             ELSE NULL END,
        'Access granted'::TEXT;
END;
$$;
...
CREATE OR REPLACE FUNCTION public.can_request_tier(p_contact_id uuid, p_tier_id uuid) RETURNS boolean
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_contact RECORD;
...
CREATE OR REPLACE FUNCTION public.can_user_perform_kb_action(p_user_id uuid, p_action character varying, p_article_id uuid DEFAULT NULL::uuid, p_category_id uuid DEFAULT NULL::uuid) RETURNS boolean
    LANGUAGE plpgsql
    AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION public.check_certificate_expiry() RETURNS integer
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_cert RECORD;
...
CREATE OR REPLACE FUNCTION public.check_provider_permission(p_provider_id uuid, p_permission character varying, p_entity_type character varying DEFAULT NULL::character varying, p_entity_id uuid DEFAULT NULL::uuid) RETURNS boolean
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_has_permission BOOLEAN := false;
...
--
-- Name: classify_user_by_domain(character varying, uuid); Type: FUNCTION; Schema: public; Owner: -
--

CREATE OR REPLACE FUNCTION public.classify_user_by_domain(p_email character varying, p_organization_id uuid) RETURNS TABLE(domain_type character varying, contact_type character varying, role_id uuid, department character varying, is_internal boolean, require_verification boolean, auto_approve boolean)
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_domain VARCHAR(255);
    v_domain_record organization_domains%ROWTYPE;
BEGIN
    ...
    IF FOUND THEN
        RETURN QUERY SELECT
            v_domain_record.domain_type,
            v_domain_record.auto_contact_type,
            v_domain_record.auto_role_id,
            v_domain_record.auto_department,
            v_domain_record.domain_type = 'internal',
            v_domain_record.require_email_verification,
            v_domain_record.auto_approve_users;
    ELSE
        ...
        RETURN QUERY SELECT
            ...
            NULL::VARCHAR(100),
            false,
            true, -- Require verification for external
            false; -- Don't auto-approve external
    END IF;
END;
$$;
...
CREATE OR REPLACE FUNCTION public.clone_job_title(p_source_id uuid, p_new_name character varying, p_new_department_id uuid DEFAULT NULL::uuid, p_created_by uuid DEFAULT NULL::uuid) RETURNS uuid
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_source RECORD;
...
CREATE OR REPLACE FUNCTION public.complete_maintenance(p_schedule_id uuid, p_performed_by uuid, p_work_performed text DEFAULT NULL::text, p_ticket_id uuid DEFAULT NULL::uuid) RETURNS boolean
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_schedule RECORD;
...
CREATE OR REPLACE FUNCTION public.create_project_from_template(p_template_id uuid, p_name character varying, p_contact_id uuid DEFAULT NULL::uuid, p_manager_id uuid DEFAULT NULL::uuid, p_start_date date DEFAULT CURRENT_DATE) RETURNS uuid
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_template RECORD;
...
CREATE OR REPLACE FUNCTION public.create_ticket_from_chat(p_session_id uuid, p_subject character varying, p_description text, p_category_id uuid DEFAULT NULL::uuid, p_priority character varying DEFAULT 'medium'::character varying, p_ai_suggested boolean DEFAULT false) RETURNS uuid
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_session RECORD;
...
--
-- Name: credential_access_checkin(uuid, character varying, character varying, character varying); Type: FUNCTION; Schema: public; Owner: -
--

CREATE OR REPLACE FUNCTION public.credential_access_checkin(p_request_id uuid, p_checked_in_by_name character varying, p_checked_in_by_email character varying, p_ip_address character varying DEFAULT NULL::character varying) RETURNS TABLE(success boolean, new_expiry timestamp with time zone, message text)
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_request credential_access_requests%ROWTYPE;
...
BEGIN
    ...
    IF NOT FOUND OR v_request.status != 'approved' THEN
        RETURN QUERY SELECT false, NULL::TIMESTAMP WITH TIME ZONE, 'Invalid or inactive access request'::TEXT;
        RETURN;
    END IF;
    
    SELECT * INTO v_policy FROM credential_sharing_policies WHERE id = v_request.policy_id;
    
    -- Check if renewals exceeded
    IF v_request.renewals_count >= COALESCE(v_policy.max_renewals, 7) THEN
        RETURN QUERY SELECT false, NULL::TIMESTAMP WITH TIME ZONE, 'Maximum renewals exceeded'::TEXT;
        RETURN;
    END IF;
    
    -- Calculate new expiry
    v_new_expiry := NOW() + (COALESCE(v_policy.access_duration_hours, 8) || ' hours')::INTERVAL;
    ...
    SET last_check_in_at = NOW(),
        ...
        access_expires_at = v_new_expiry
    WHERE id = p_request_id;
    
    -- Log check-in
    INSERT INTO credential_access_checkins (
        access_request_id, checked_in_by_name, checked_in_by_email,
        ip_address, access_extended_until, is_renewal
    ) VALUES (
        p_request_id, p_checked_in_by_name, p_checked_in_by_email,
        p_ip_address, v_new_expiry, true
    );
    
    RETURN QUERY SELECT true, v_new_expiry, 'Access extended'::TEXT;
END;
$$;
...
CREATE OR REPLACE FUNCTION public.detect_dns_changes(p_domain_id uuid, p_new_snapshot_id uuid) RETURNS integer
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_domain RECORD;
...
CREATE OR REPLACE FUNCTION public.disable_feature(p_organization_id uuid, p_feature_key character varying, p_user_id uuid) RETURNS jsonb
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_feature RECORD;
...
CREATE OR REPLACE FUNCTION public.enable_feature(p_organization_id uuid, p_feature_key character varying, p_user_id uuid, p_acknowledge_beta boolean DEFAULT false) RETURNS jsonb
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_feature RECORD;
...
CREATE OR REPLACE FUNCTION public.evaluate_dynamic_group(p_group_id uuid) RETURNS TABLE(added integer, removed integer, unchanged integer)
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_group RECORD;
...
CREATE OR REPLACE FUNCTION public.evaluate_routing_rules(p_ticket_id uuid, p_workspace_id uuid) RETURNS TABLE(rule_id uuid, rule_name character varying, team_id uuid, user_id uuid, priority character varying, tags text[])
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_ticket RECORD;
...
CREATE OR REPLACE FUNCTION public.expire_stale_credential_access() RETURNS integer
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_expired_count INT;
...
CREATE OR REPLACE FUNCTION public.find_category_matches(p_workspace_id uuid, p_subject text, p_description text, p_limit integer DEFAULT 5) RETURNS TABLE(category_id uuid, category_name character varying, category_path text, base_type public.request_category, keyword_score integer, total_score numeric)
    LANGUAGE plpgsql
    AS $$
BEGIN
    RETURN QUERY
...
CREATE OR REPLACE FUNCTION public.generate_asset_name(p_organization_id uuid, p_asset_type_id uuid, p_location character varying DEFAULT NULL::character varying, p_department character varying DEFAULT NULL::character varying) RETURNS character varying
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_template RECORD;
...
CREATE OR REPLACE FUNCTION public.generate_due_recurring_tickets() RETURNS TABLE(recurring_ticket_id uuid, ticket_id uuid, scheduled_for date)
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_rt RECORD;
...
CREATE OR REPLACE FUNCTION public.generate_offboarding_tasks(p_request_id uuid) RETURNS integer
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_request RECORD;
...
CREATE OR REPLACE FUNCTION public.generate_offboarding_tasks_v2(p_request_id uuid) RETURNS integer
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_request RECORD;
...
CREATE OR REPLACE FUNCTION public.generate_onboarding_tasks(p_request_id uuid) RETURNS integer
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_request RECORD;
...
CREATE OR REPLACE FUNCTION public.generate_onboarding_tasks_v2(p_request_id uuid) RETURNS integer
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_request RECORD;
...
CREATE OR REPLACE FUNCTION public.get_ai_redacted_fields(p_session_id uuid, p_resource_type character varying) RETURNS text[]
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
DECLARE
    v_session RECORD;
...
CREATE OR REPLACE FUNCTION public.get_all_reports(manager_id uuid) RETURNS TABLE(contact_id uuid, depth integer)
    LANGUAGE plpgsql
    AS $$
BEGIN
    RETURN QUERY
...
CREATE OR REPLACE FUNCTION public.get_available_agents(p_team_id uuid, p_workspace_id uuid DEFAULT NULL::uuid) RETURNS TABLE(user_id uuid, name text, current_tickets integer, max_tickets integer, availability_score numeric)
    LANGUAGE plpgsql
    AS $$
BEGIN
    RETURN QUERY
...
CREATE OR REPLACE FUNCTION public.get_available_tiers_for_contact(p_contact_id uuid) RETURNS TABLE(tier_id uuid, tier_name character varying, category character varying, description text, requires_approval boolean, available_count bigint, show_count boolean)
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_contact RECORD;
...
CREATE OR REPLACE FUNCTION public.get_contextual_help(p_feature_key character varying, p_context character varying, p_user_id uuid) RETURNS TABLE(id uuid, title character varying, content text, learn_more_url character varying, video_url character varying)
    LANGUAGE plpgsql
    AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION public.get_department_tree(p_organization_id uuid) RETURNS TABLE(id uuid, name character varying, code character varying, path text, depth integer, parent_id uuid, head_name text, employee_count bigint, children_count bigint)
    LANGUAGE plpgsql
    AS $$
BEGIN
    RETURN QUERY
...
CREATE OR REPLACE FUNCTION public.get_direct_reports_count(manager_id uuid) RETURNS integer
    LANGUAGE plpgsql
    AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION public.get_location_map_data(p_organization_id uuid) RETURNS TABLE(id uuid, name character varying, location_level character varying, latitude numeric, longitude numeric, employee_count bigint, asset_count bigint, address text, is_remote boolean)
    LANGUAGE plpgsql
    AS $$
BEGIN
    RETURN QUERY
...
CREATE OR REPLACE FUNCTION public.get_org_chart(p_organization_id uuid, p_root_contact_id uuid DEFAULT NULL::uuid) RETURNS TABLE(id uuid, name text, job_title text, department text, location text, email character varying, avatar_url character varying, reports_to_id uuid, direct_reports_count bigint, depth integer)
    LANGUAGE plpgsql
    AS $$
BEGIN
    RETURN QUERY
...
CREATE OR REPLACE FUNCTION public.get_pending_acknowledgments(p_user_id uuid DEFAULT NULL::uuid, p_contact_id uuid DEFAULT NULL::uuid) RETURNS TABLE(article_id uuid, title character varying, category_name character varying, acknowledgment_required_by date, days_overdue integer)
    LANGUAGE plpgsql
    AS $$
BEGIN
    RETURN QUERY
    SELECT
        ...
        CASE 
            WHEN a.acknowledgment_required_by < CURRENT_DATE 
            THEN (CURRENT_DATE - a.acknowledgment_required_by)::INT
            ELSE 0
        END as days_overdue
    FROM kb_articles a
    ...
    WHERE a.requires_acknowledgment = true
      ...
      -- Check visibility (simplified - user should be able to see the article)
      AND (
          a.visibility IN ('public', 'authenticated')
          OR (a.visibility = 'internal' AND p_user_id IS NOT NULL)
      )
    ORDER BY a.acknowledgment_required_by ASC NULLS LAST, a.title;
END;
$$;
...
CREATE OR REPLACE FUNCTION public.get_pending_emails(p_limit integer DEFAULT 50) RETURNS TABLE(id uuid, to_email character varying, to_name character varying, subject character varying, body_text text, body_html text, attachments jsonb)
    LANGUAGE plpgsql
    AS $$
BEGIN
    RETURN QUERY
...
CREATE OR REPLACE FUNCTION public.get_pending_policies(p_user_id uuid DEFAULT NULL::uuid, p_contact_id uuid DEFAULT NULL::uuid) RETURNS TABLE(policy_id uuid, policy_name character varying, policy_type public.policy_type, version character varying, due_date date, days_until_due integer, is_overdue boolean)
    LANGUAGE plpgsql
    AS $$
BEGIN
    RETURN QUERY
...
CREATE OR REPLACE FUNCTION public.get_subordinates(manager_uuid uuid) RETURNS TABLE(user_id uuid, depth integer)
    LANGUAGE sql STABLE
    AS $$
WITH RECURSIVE subordinates AS (
    -- Direct reports
    SELECT mr.user_id, 1 as depth
    FROM manager_relationships mr
    WHERE mr.manager_id = manager_uuid
    AND mr.is_active = true
    AND mr.relationship_type = 'direct'
    ...
    -- Indirect reports
    SELECT mr.user_id, s.depth + 1
    FROM manager_relationships mr
    INNER JOIN subordinates s ON mr.manager_id = s.user_id
    WHERE mr.is_active = true
    AND mr.relationship_type = 'direct'
    AND s.depth < 10 -- Prevent infinite loops
)
...
CREATE OR REPLACE FUNCTION public.get_suggested_articles_for_ticket(p_ticket_id uuid, p_limit integer DEFAULT 5) RETURNS TABLE(article_id uuid, title character varying, slug character varying, relevance_source character varying)
    LANGUAGE plpgsql
    AS $$
BEGIN
    RETURN QUERY
...
CREATE OR REPLACE FUNCTION public.get_user_ui_mode(p_user_id uuid) RETURNS character varying
    LANGUAGE plpgsql
    AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION public.handle_ticket_status_change() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION public.has_acknowledged_required_policies(p_user_id uuid DEFAULT NULL::uuid, p_contact_id uuid DEFAULT NULL::uuid, p_for_onboarding boolean DEFAULT false) RETURNS boolean
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_pending_count INT;
...
CREATE OR REPLACE FUNCTION public.initialize_ticket_statuses(org_id uuid) RETURNS void
    LANGUAGE plpgsql
    AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION public.is_feature_enabled(p_organization_id uuid, p_feature character varying) RETURNS boolean
    LANGUAGE plpgsql
    AS $_$
DECLARE
    v_enabled BOOLEAN;
...
CREATE OR REPLACE FUNCTION public.is_feature_enabled_v2(p_organization_id uuid, p_feature_key character varying) RETURNS boolean
    LANGUAGE plpgsql
    AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION public.log_ai_data_access(p_session_id uuid, p_resource_type character varying, p_resource_id uuid, p_action character varying, p_query_text text, p_results_count integer, p_access_granted boolean, p_denial_reason text DEFAULT NULL::text, p_redacted_fields text[] DEFAULT NULL::text[]) RETURNS uuid
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_log_id UUID;
...
--
-- Name: log_credential_reveal(uuid, uuid, character varying, uuid, uuid, character varying, character varying, character varying, text[], character varying, text, boolean, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE OR REPLACE FUNCTION public.log_credential_reveal(p_credential_id uuid, p_access_request_id uuid DEFAULT NULL::uuid, p_revealed_by_type character varying DEFAULT 'user'::character varying, p_revealed_by_user_id uuid DEFAULT NULL::uuid, p_revealed_by_api_key_id uuid DEFAULT NULL::uuid, p_revealed_by_name character varying DEFAULT NULL::character varying, p_revealed_by_email character varying DEFAULT NULL::character varying, p_reveal_method character varying DEFAULT 'ui_click'::character varying, p_fields_revealed text[] DEFAULT ARRAY['password'::text], p_ip_address character varying DEFAULT NULL::character varying, p_user_agent text DEFAULT NULL::text, p_is_break_glass boolean DEFAULT false, p_break_glass_justification text DEFAULT NULL::text) RETURNS uuid
    LANGUAGE plpgsql
    AS $$
DECLARE
...
BEGIN
    ...
    -- Check reveal limits if via access request
    IF p_access_request_id IS NOT NULL THEN
        SELECT csp.* INTO v_policy
        FROM credential_access_requests car
        JOIN credential_sharing_policies csp ON car.policy_id = csp.id
        WHERE car.id = p_access_request_id;
    ...
    INSERT INTO credential_reveal_log (
        ...
        access_request_id,
        revealed_by_type, revealed_by_user_id, revealed_by_api_key_id,
        revealed_by_name, revealed_by_email,
        reveal_method, fields_revealed,
        ip_address, user_agent,
        is_break_glass, break_glass_justification
    ) VALUES (
        v_credential.organization_id, p_credential_id, v_credential.name,
        p_access_request_id,
        p_revealed_by_type, p_revealed_by_user_id, p_revealed_by_api_key_id,
        p_revealed_by_name, p_revealed_by_email,
        p_reveal_method, p_fields_revealed,
        p_ip_address, p_user_agent,
        p_is_break_glass, p_break_glass_justification
    )
    RETURNING id INTO v_log_id;
    
    -- Create break-glass incident if needed
    IF p_is_break_glass THEN
        INSERT INTO break_glass_incidents (
            organization_id, credential_id, credential_name,
            provider_api_key_id, accessor_name, accessor_email,
            justification, ip_address, user_agent
        ) VALUES (
            v_credential.organization_id, p_credential_id, v_credential.name,
            p_revealed_by_api_key_id, p_revealed_by_name, p_revealed_by_email,
            p_break_glass_justification, p_ip_address, p_user_agent
        );
    END IF;
...
--
-- Name: log_provider_action(uuid, character varying, character varying, uuid, character varying, character varying, character varying, integer, boolean, character varying, text, character varying, character varying, text[], text[], jsonb, jsonb, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE OR REPLACE FUNCTION public.log_provider_action(p_api_key_id uuid, p_action character varying, p_resource_type character varying, p_resource_id uuid, p_resource_name character varying, p_request_method character varying, p_request_path character varying, p_response_status integer, p_response_success boolean, p_ip_address character varying, p_user_agent text, p_provider_user_name character varying DEFAULT NULL::character varying, p_provider_user_email character varying DEFAULT NULL::character varying, p_fields_accessed text[] DEFAULT NULL::text[], p_fields_modified text[] DEFAULT NULL::text[], p_old_values jsonb DEFAULT NULL::jsonb, p_new_values jsonb DEFAULT NULL::jsonb, p_error_message text DEFAULT NULL::text) RETURNS uuid
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_log_id UUID;
    v_key provider_api_keys%ROWTYPE;
BEGIN
    SELECT * INTO v_key FROM provider_api_keys WHERE id = p_api_key_id;
    
    INSERT INTO provider_action_log (
        organization_id, api_key_id, provider_name,
        provider_user_name, provider_user_email,
        action, resource_type, resource_id, resource_name,
        request_method, request_path,
        response_status, response_success, error_message,
        ip_address, user_agent,
        fields_accessed, fields_modified, old_values, new_values,
        completed_at
    ) VALUES (
        v_key.organization_id, p_api_key_id, v_key.provider_name,
        p_provider_user_name, p_provider_user_email,
        p_action, p_resource_type, p_resource_id, p_resource_name,
        p_request_method, p_request_path,
        p_response_status, p_response_success, p_error_message,
        p_ip_address, p_user_agent,
        p_fields_accessed, p_fields_modified, p_old_values, p_new_values,
        NOW()
    )
    RETURNING id INTO v_log_id;
...
--
-- Name: log_provider_contribution(uuid, uuid, character varying, character varying, character varying, character varying, uuid, character varying, text[], jsonb, jsonb, text, uuid, character varying); Type: FUNCTION; Schema: public; Owner: -
--

CREATE OR REPLACE FUNCTION public.log_provider_contribution(p_organization_id uuid, p_provider_api_key_id uuid, p_contributor_name character varying, p_contributor_email character varying, p_action character varying, p_entity_type character varying, p_entity_id uuid, p_entity_name character varying, p_fields_changed text[] DEFAULT NULL::text[], p_old_values jsonb DEFAULT NULL::jsonb, p_new_values jsonb DEFAULT NULL::jsonb, p_change_summary text DEFAULT NULL::text, p_related_ticket_id uuid DEFAULT NULL::uuid, p_ip_address character varying DEFAULT NULL::character varying) RETURNS uuid
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_key provider_api_keys%ROWTYPE;
...
BEGIN
    SELECT * INTO v_key FROM provider_api_keys WHERE id = p_provider_api_key_id;
    
    INSERT INTO provider_contributions (
        organization_id, provider_api_key_id, provider_name,
        contributor_name, contributor_email,
        action, entity_type, entity_id, entity_name,
        fields_changed, old_values, new_values, change_summary,
        related_ticket_id, ip_address
    ) VALUES (
        p_organization_id, p_provider_api_key_id, v_key.provider_name,
        p_contributor_name, p_contributor_email,
        p_action, p_entity_type, p_entity_id, p_entity_name,
        p_fields_changed, p_old_values, p_new_values, p_change_summary,
        p_related_ticket_id, p_ip_address
    )
    RETURNING id INTO v_contribution_id;
    
    -- Update the entity's last_modified_by_provider fields
    CASE p_entity_type
        WHEN 'ticket' THEN
            UPDATE tickets SET 
                last_modified_by_provider_id = p_provider_api_key_id,
                last_modified_by_provider_name = p_contributor_name || ' (' || v_key.provider_name || ')'
...
--
-- Name: provider_save_credential(uuid, uuid, character varying, character varying, character varying, character varying, text, character varying, text, character varying, character varying, text, uuid, character varying); Type: FUNCTION; Schema: public; Owner: -
--

CREATE OR REPLACE FUNCTION public.provider_save_credential(p_organization_id uuid, p_provider_api_key_id uuid, p_contributor_name character varying, p_contributor_email character varying, p_name character varying, p_username character varying, p_password text, p_url character varying DEFAULT NULL::character varying, p_notes text DEFAULT NULL::text, p_service_name character varying DEFAULT NULL::character varying, p_action character varying DEFAULT 'created'::character varying, p_reason text DEFAULT NULL::text, p_related_ticket_id uuid DEFAULT NULL::uuid, p_ip_address character varying DEFAULT NULL::character varying) RETURNS uuid
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_key provider_api_keys%ROWTYPE;
...
BEGIN
    ...
    IF v_credential_id IS NOT NULL THEN
        ...
        SET username = COALESCE(p_username, username),
            password_encrypted = p_password, -- Should be encrypted by caller
    ...
    ELSE
        ...
        INSERT INTO credentials (
            organization_id, name, username, password_encrypted, url, notes,
            created_by_provider_id, created_by_provider_name
        ) VALUES (
            p_organization_id, p_name, p_username, p_password, p_url, p_notes,
            p_provider_api_key_id, p_contributor_name || ' (' || v_key.provider_name || ')'
        )
        RETURNING id INTO v_credential_id;
    END IF;
    
    -- Log the contribution
    INSERT INTO credential_contributions (
        organization_id, credential_id, action,
        provider_api_key_id, provider_name,
        contributor_name, contributor_email,
        service_name, account_identifier,
        reason, related_ticket_id, ip_address
    ) VALUES (
        p_organization_id, v_credential_id, p_action,
        p_provider_api_key_id, v_key.provider_name,
        p_contributor_name, p_contributor_email,
        p_service_name, p_username,
        p_reason, p_related_ticket_id, p_ip_address
    );
    
    -- Also log in general contributions
    INSERT INTO provider_contributions (
        organization_id, provider_api_key_id, provider_name,
        contributor_name, contributor_email,
        action, entity_type, entity_id, entity_name,
        change_summary, related_ticket_id, ip_address
    ) VALUES (
        p_organization_id, p_provider_api_key_id, v_key.provider_name,
        p_contributor_name, p_contributor_email,
        CASE WHEN p_action = 'password_reset' THEN 'update' ELSE 'create' END,
        'credential', v_credential_id, p_name,
        CASE 
            WHEN p_action = 'password_reset' THEN 'Password reset for ' || COALESCE(p_service_name, p_name)
            ELSE 'Created credential for ' || COALESCE(p_service_name, p_name)
        END,
        p_related_ticket_id, p_ip_address
    );
...
--
-- Name: provider_soft_delete(character varying, uuid, uuid, character varying, character varying, text, character varying); Type: FUNCTION; Schema: public; Owner: -
--

CREATE OR REPLACE FUNCTION public.provider_soft_delete(p_entity_type character varying, p_entity_id uuid, p_provider_api_key_id uuid, p_contributor_name character varying, p_contributor_email character varying, p_reason text, p_ip_address character varying DEFAULT NULL::character varying) RETURNS boolean
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_key provider_api_keys%ROWTYPE;
...
BEGIN
    ...
    CASE p_entity_type
        WHEN 'ticket' THEN
            ...
            SET is_deleted = true,
                deleted_at = NOW(),
                deleted_by_provider_id = p_provider_api_key_id,
                deleted_by_name = p_contributor_name || ' (' || v_key.provider_name || ')',
                delete_reason = p_reason
            WHERE id = p_entity_id AND is_deleted = false
            RETURNING subject, organization_id INTO v_entity_name, v_org_id;
        ...
        WHEN 'asset' THEN
            ...
            SET is_deleted = true,
                ...
                delete_reason = p_reason
            WHERE id = p_entity_id AND is_deleted = false
            RETURNING name, organization_id INTO v_entity_name, v_org_id;
        ...
        WHEN 'contact' THEN
            ...
            SET is_deleted = true,
                ...
                delete_reason = p_reason
            WHERE id = p_entity_id AND is_deleted = false
            RETURNING COALESCE(first_name || ' ' || last_name, email), organization_id INTO v_entity_name, v_org_id;
        ...
        WHEN 'kb_article' THEN
            ...
            SET is_deleted = true,
                ...
                delete_reason = p_reason
            WHERE id = p_entity_id AND is_deleted = false
            RETURNING title, organization_id INTO v_entity_name, v_org_id;
    ...
    INSERT INTO provider_contributions (
        ...
        contributor_name, contributor_email,
        action, entity_type, entity_id, entity_name,
        delete_reason, ip_address
    ) VALUES (
        v_org_id, p_provider_api_key_id, v_key.provider_name,
        p_contributor_name, p_contributor_email,
        'soft_delete', p_entity_type, p_entity_id, v_entity_name,
...
CREATE OR REPLACE FUNCTION public.renew_certificate(p_certificate_id uuid, p_new_expires_at date, p_new_serial_number character varying DEFAULT NULL::character varying, p_new_public_key text DEFAULT NULL::text, p_user_id uuid DEFAULT NULL::uuid) RETURNS boolean
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_cert RECORD;
...
CREATE OR REPLACE FUNCTION public.request_chat_handoff(p_session_id uuid, p_reason character varying, p_reason_details text DEFAULT NULL::text) RETURNS uuid
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_handoff_id UUID;
...
--
-- Name: request_credential_access(uuid, uuid, character varying, character varying, text, character varying, character varying); Type: FUNCTION; Schema: public; Owner: -
--

CREATE OR REPLACE FUNCTION public.request_credential_access(p_credential_id uuid, p_api_key_id uuid, p_requester_name character varying, p_requester_email character varying, p_justification text, p_ticket_reference character varying DEFAULT NULL::character varying, p_ip_address character varying DEFAULT NULL::character varying) RETURNS TABLE(success boolean, request_id uuid, status public.credential_request_status, message text)
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_policy credential_sharing_policies%ROWTYPE;
...
BEGIN
    ...
    SELECT * INTO v_credential FROM credentials WHERE id = p_credential_id;
    IF NOT FOUND THEN
        RETURN QUERY SELECT false, NULL::UUID, NULL::credential_request_status, 'Credential not found'::TEXT;
        RETURN;
    END IF;
    ...
    SELECT * INTO v_key FROM provider_api_keys WHERE id = p_api_key_id;
    IF NOT FOUND OR NOT v_key.is_active THEN
        RETURN QUERY SELECT false, NULL::UUID, NULL::credential_request_status, 'Invalid API key'::TEXT;
        RETURN;
    END IF;
    
    -- Get sharing policy (specific to this provider, or default)
    SELECT * INTO v_policy 
    FROM credential_sharing_policies 
    WHERE credential_id = p_credential_id 
      AND (provider_api_key_id = p_api_key_id OR provider_api_key_id IS NULL)
    ...
    IF NOT FOUND OR v_policy.share_mode = 'never' THEN
        RETURN QUERY SELECT false, NULL::UUID, NULL::credential_request_status, 'Credential sharing not enabled'::TEXT;
        RETURN;
    END IF;
    
    -- Check for existing active request
    IF EXISTS (
        SELECT 1 FROM credential_access_requests
        WHERE credential_id = p_credential_id
          AND provider_api_key_id = p_api_key_id
          AND status IN ('pending', 'approved')
          AND (access_expires_at IS NULL OR access_expires_at > NOW())
    ) THEN
        RETURN QUERY SELECT false, NULL::UUID, NULL::credential_request_status, 'Active access request already exists'::TEXT;
        RETURN;
    END IF;
    
    -- Calculate expiry
    v_expires_at := NOW() + (v_policy.approval_timeout_minutes || ' minutes')::INTERVAL;
    
    -- Create request
    INSERT INTO credential_access_requests (
        organization_id, credential_id, policy_id,
        provider_api_key_id, provider_name,
        requester_name, requester_email,
        request_mode, justification, ticket_reference,
        status, request_ip, expires_at
    ) VALUES (
        v_credential.organization_id, p_credential_id, v_policy.id,
        p_api_key_id, v_key.provider_name,
        p_requester_name, p_requester_email,
        v_policy.share_mode, p_justification, p_ticket_reference,
        CASE WHEN v_policy.requires_approval THEN 'pending' ELSE 'approved' END,
        p_ip_address, v_expires_at
    )
    RETURNING id INTO v_request_id;
    
    -- If auto-approved, set access window
    IF NOT v_policy.requires_approval THEN
        UPDATE credential_access_requests
        SET access_granted_at = NOW(),
            access_expires_at = NOW() + (v_policy.access_duration_hours || ' hours')::INTERVAL
    ...
    RETURN QUERY SELECT 
        true, 
        v_request_id, 
        CASE WHEN v_policy.requires_approval THEN 'pending'::credential_request_status ELSE 'approved'::credential_request_status END,
        CASE WHEN v_policy.requires_approval THEN 'Request submitted for approval' ELSE 'Access granted' END;
END;
$$;
...
CREATE OR REPLACE FUNCTION public.resolve_dns_alert(p_alert_id uuid, p_ticket_id uuid, p_user_id uuid, p_resolution_notes text DEFAULT NULL::text) RETURNS boolean
    LANGUAGE plpgsql
    AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION public.restore_deleted_item(p_entity_type character varying, p_entity_id uuid, p_restored_by_user_id uuid) RETURNS boolean
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_entity_name VARCHAR(500);
...
CREATE OR REPLACE FUNCTION public.revoke_api_key(p_api_key_id uuid, p_revoked_by uuid, p_reason text) RETURNS boolean
    LANGUAGE plpgsql
    AS $$
BEGIN
    -- Revoke the key
...
CREATE OR REPLACE FUNCTION public.search_documents(p_organization_id uuid, p_query text, p_limit integer DEFAULT 50) RETURNS TABLE(id uuid, title character varying, description text, folder_id uuid, company_id uuid, rank real)
    LANGUAGE plpgsql
    AS $$
BEGIN
    RETURN QUERY
...
CREATE OR REPLACE FUNCTION public.search_kb_articles_for_user(p_organization_id uuid, p_query text, p_user_id uuid DEFAULT NULL::uuid, p_contact_id uuid DEFAULT NULL::uuid, p_category_id uuid DEFAULT NULL::uuid, p_limit integer DEFAULT 10) RETURNS TABLE(id uuid, title character varying, summary text, slug character varying, category_id uuid, category_name character varying, visibility character varying, relevance_score real, view_count integer, helpful_ratio numeric, requires_acknowledgment boolean, is_acknowledged boolean)
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_user_role VARCHAR(50);
    ...
    v_contact_department VARCHAR(100);
    v_contact_job_title VARCHAR(100);
BEGIN
    -- Get user/contact attributes for filtering
    IF p_user_id IS NOT NULL THEN
    ...
    WHERE a.organization_id = p_organization_id
      ...
      AND (
          ...
          -- Internal = staff only
          OR (a.visibility = 'internal' AND p_user_id IS NOT NULL AND v_user_role IN ('admin', 'technician', 'manager'))
          -- Private = check specific access lists
          OR (
              a.visibility = 'private'
              AND (
                  -- Check role access
                  (p_user_id IS NOT NULL AND a.visible_to_roles && ARRAY[(SELECT role_id FROM users WHERE users.id = p_user_id)])
                  ...
                  OR (v_contact_department IS NOT NULL AND v_contact_department = ANY(a.visible_to_departments))
                  -- Check job title access
                  OR (v_contact_job_title IS NOT NULL AND v_contact_job_title = ANY(a.visible_to_job_titles))
              )
          )
      )
    ORDER BY relevance_score DESC, a.view_count DESC
...
CREATE OR REPLACE FUNCTION public.seed_asset_master_data(p_organization_id uuid) RETURNS void
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_type_id UUID;
...
CREATE OR REPLACE FUNCTION public.seed_default_categories(p_workspace_id uuid, p_workspace_type character varying DEFAULT 'it'::character varying) RETURNS integer
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_org_id UUID;
...
CREATE OR REPLACE FUNCTION public.send_notification(p_user_id uuid, p_title character varying, p_body text, p_notification_type character varying DEFAULT 'info'::character varying, p_event_type character varying DEFAULT NULL::character varying, p_entity_type character varying DEFAULT NULL::character varying, p_entity_id uuid DEFAULT NULL::uuid, p_link character varying DEFAULT NULL::character varying, p_actions jsonb DEFAULT NULL::jsonb, p_send_email boolean DEFAULT true) RETURNS uuid
    LANGUAGE plpgsql
    AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION public.set_domain_history_severity() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
...
--
-- Name: should_send_email(character varying, character varying); Type: FUNCTION; Schema: public; Owner: -
--

CREATE OR REPLACE FUNCTION public.should_send_email(p_recipient character varying, p_email_type character varying) RETURNS boolean
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_prefs email_preferences%ROWTYPE;
BEGIN
    -- Find preferences for this recipient
    SELECT * INTO v_prefs
    FROM email_preferences
    WHERE email = p_recipient
       OR contact_id IN (SELECT id FROM contacts WHERE email = p_recipient)
       OR user_id IN (SELECT id FROM users WHERE email = p_recipient)
    LIMIT 1;
    ...
    CASE p_email_type
        WHEN 'ticket_notification' THEN RETURN v_prefs.ticket_notifications;
        WHEN 'ticket_reply' THEN RETURN v_prefs.ticket_replies;
        WHEN 'sla_warning' THEN RETURN v_prefs.sla_warnings;
        WHEN 'system' THEN RETURN v_prefs.system_notifications;
        WHEN 'marketing' THEN RETURN v_prefs.marketing;
        ELSE RETURN true;
    END CASE;
END;
$$;
...
--
-- Name: submit_kb_article_for_review(uuid, uuid); Type: FUNCTION; Schema: public; Owner: -
--

CREATE OR REPLACE FUNCTION public.submit_kb_article_for_review(p_article_id uuid, p_user_id uuid) RETURNS boolean
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_contributor kb_contributors%ROWTYPE;
BEGIN
    -- Get contributor info
    SELECT * INTO v_contributor
    FROM kb_contributors
    WHERE user_id = p_user_id AND is_active = true;
    
    IF NOT FOUND THEN
        RAISE EXCEPTION 'User is not an active KB contributor';
    END IF;
    
    -- Check if contributor can self-publish
    IF v_contributor.can_self_publish THEN
        UPDATE kb_articles
...
CREATE OR REPLACE FUNCTION public.trigger_update_project_progress() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION public.update_accessory_quantities() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION public.update_category_path() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_parent_path TEXT;
...
CREATE OR REPLACE FUNCTION public.update_department_path() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_parent_path TEXT;
...
CREATE OR REPLACE FUNCTION public.update_group_path() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
    IF NEW.parent_id IS NULL THEN
...
CREATE OR REPLACE FUNCTION public.update_kb_category_count() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION public.update_location_path() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_parent_path TEXT;
...
CREATE OR REPLACE FUNCTION public.update_project_progress(p_project_id uuid) RETURNS void
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_total_tasks INT;
...
--
-- Name: validate_api_key(character varying, character varying, character varying); Type: FUNCTION; Schema: public; Owner: -
--

CREATE OR REPLACE FUNCTION public.validate_api_key(p_key_hash character varying, p_required_scope character varying, p_ip_address character varying DEFAULT NULL::character varying) RETURNS TABLE(is_valid boolean, api_key_id uuid, organization_id uuid, provider_name character varying, error_message text)
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_key provider_api_keys%ROWTYPE;
    v_contract_ack provider_contract_acknowledgments%ROWTYPE;
    v_scope api_scopes%ROWTYPE;
BEGIN
    -- Find the key
    SELECT * INTO v_key
    ...
    IF NOT FOUND THEN
        RETURN QUERY SELECT false, NULL::UUID, NULL::UUID, NULL::VARCHAR(200), 'Invalid API key'::TEXT;
        RETURN;
    END IF;
    
    -- Check if active
    IF NOT v_key.is_active OR v_key.is_revoked THEN
        RETURN QUERY SELECT false, v_key.id, v_key.organization_id, v_key.provider_name, 'API key is revoked or inactive'::TEXT;
        RETURN;
    END IF;
    
    -- Check expiry
    IF v_key.expires_at IS NOT NULL AND v_key.expires_at < NOW() THEN
        RETURN QUERY SELECT false, v_key.id, v_key.organization_id, v_key.provider_name, 'API key has expired'::TEXT;
        RETURN;
    END IF;
    
    -- Check IP whitelist
    IF v_key.allowed_ips IS NOT NULL AND p_ip_address IS NOT NULL THEN
        IF NOT p_ip_address = ANY(v_key.allowed_ips) THEN
            RETURN QUERY SELECT false, v_key.id, v_key.organization_id, v_key.provider_name, 'IP address not allowed'::TEXT;
            RETURN;
        END IF;
    END IF;
    ...
    IF p_required_scope IS NOT NULL AND NOT p_required_scope = ANY(v_key.scopes) THEN
        RETURN QUERY SELECT false, v_key.id, v_key.organization_id, v_key.provider_name, 'Insufficient permissions (missing scope: ' || p_required_scope || ')'::TEXT;
        RETURN;
    END IF;
    
    -- Check contract acknowledgment if required
    IF v_key.requires_contract AND v_key.required_contract_id IS NOT NULL THEN
        SELECT * INTO v_contract_ack
        FROM provider_contract_acknowledgments
        WHERE api_key_id = v_key.id
          AND contract_id = v_key.required_contract_id
          AND (expires_at IS NULL OR expires_at > NOW());
        
        IF NOT FOUND THEN
            RETURN QUERY SELECT false, v_key.id, v_key.organization_id, v_key.provider_name, 'Contract acknowledgment required'::TEXT;
            RETURN;
        END IF;
    END IF;
    ...
    -- Valid!
    RETURN QUERY SELECT true, v_key.id, v_key.organization_id, v_key.provider_name, NULL::TEXT;
END;
$$;
...
CREATE OR REPLACE FUNCTION public.validate_ticket_closure(p_ticket_id uuid, p_user_id uuid) RETURNS TABLE(can_close boolean, errors text[])
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_ticket RECORD;
...
CREATE TABLE IF NOT EXISTS public.access_profile_items (
...
CREATE TABLE IF NOT EXISTS public.access_request_comments (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
...
CREATE SEQUENCE IF NOT EXISTS public.access_requests_request_number_seq
    AS integer
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
...
CREATE TABLE IF NOT EXISTS public.accessory_disposals (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    organization_id uuid NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.accessory_inventory_transactions (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    organization_id uuid NOT NULL,
    accessory_id uuid NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.accessory_purchase_order_items (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.accessory_purchase_orders (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    organization_id uuid NOT NULL,
...
CREATE VIEW public.accessory_model_performance AS
 SELECT am.organization_id,
    ...
    ( SELECT COALESCE(sum(ad.quantity), (0)::bigint) AS "coalesce"
          ...
          WHERE ((a.model_id = am.id) AND ((ad.disposal_type)::text = ANY ((ARRAY['lost'::character varying, 'stolen'::character varying, 'not_returned'::character varying])::text[])))) AS total_lost,
    ( SELECT COALESCE(sum(ad.quantity), (0)::bigint) AS "coalesce"
           FROM (public.accessory_disposals ad
             JOIN public.accessories a ON ((ad.accessory_id = a.id)))
          WHERE ((a.model_id = am.id) AND ((ad.disposal_type)::text = ANY ((ARRAY['damaged'::character varying, 'broken'::character varying])::text[])))) AS total_broken,
        CASE
            WHEN (( SELECT count(*) AS count
              ...
              WHERE ((a.model_id = am.id) AND ((ua.status)::text <> 'active'::text))) > 0) THEN round(((( SELECT (count(*))::numeric AS count
               FROM (public.user_assignments ua
                 JOIN public.accessories a ON (((ua.resource_id = a.id) AND ((ua.resource_type)::text = 'accessory'::text))))
              WHERE ((a.model_id = am.id) AND ((ua.status)::text <> ALL ((ARRAY['active'::character varying, 'returned'::character varying])::text[])))) * 100.0) / (( SELECT count(*) AS count
               FROM (public.user_assignments ua
                 JOIN public.accessories a ON (((ua.resource_id = a.id) AND ((ua.resource_type)::text = 'accessory'::text))))
...
CREATE VIEW public.accessory_spending_analytics AS
 SELECT a.organization_id,
    ...
    ( SELECT COALESCE(sum(ad.quantity), (0)::bigint) AS "coalesce"
           FROM public.accessory_disposals ad
          WHERE ((ad.accessory_id = a.id) AND ((ad.disposal_type)::text = ANY ((ARRAY['lost'::character varying, 'stolen'::character varying, 'not_returned'::character varying])::text[])))) AS total_lost,
    ( SELECT COALESCE(sum(ad.quantity), (0)::bigint) AS "coalesce"
           FROM public.accessory_disposals ad
          WHERE ((ad.accessory_id = a.id) AND ((ad.disposal_type)::text = ANY ((ARRAY['damaged'::character varying, 'broken'::character varying])::text[])))) AS total_damaged
...
CREATE TABLE IF NOT EXISTS public.ai_category_matches (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.ai_chat_data_access_log (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    session_id uuid NOT NULL,
    resource_type character varying(50) NOT NULL,
    resource_id uuid,
...
CREATE TABLE IF NOT EXISTS public.ai_data_access_policies (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    organization_id uuid NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.ai_triage_config (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    workspace_id uuid NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.alert_correlation_rules (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    organization_id uuid NOT NULL,
    name character varying(255) NOT NULL,
    description text,
...
CREATE TABLE IF NOT EXISTS public.asset_contacts (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.asset_import_jobs (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    organization_id uuid NOT NULL,
    name character varying(255),
...
CREATE TABLE IF NOT EXISTS public.asset_naming_templates (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    organization_id uuid NOT NULL,
    asset_type_id uuid,
...
CREATE TABLE IF NOT EXISTS public.asset_request_tiers (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    organization_id uuid NOT NULL,
    name character varying(100) NOT NULL,
    description text,
...
CREATE SEQUENCE IF NOT EXISTS public.asset_requests_request_number_seq
    AS integer
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
...
CREATE TABLE IF NOT EXISTS public.asset_retrieval_assignments (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.asset_status_requestability (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    organization_id uuid NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.asset_tickets (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    asset_id uuid NOT NULL,
    ticket_id uuid NOT NULL,
...
CREATE VIEW public.asset_tier_inventory AS
 SELECT t.id AS tier_id,
    ...
    count(a.id) FILTER (WHERE ((a.status)::text = ANY ((ARRAY['In Stock'::character varying, 'Available'::character varying, 'Ready for Deployment'::character varying])::text[]))) AS available_count,
    count(a.id) FILTER (WHERE ((a.status)::text = 'In Use'::text)) AS in_use_count,
    jsonb_agg(DISTINCT jsonb_build_object('brand', a.make, 'model', a.model, 'count', 1)) FILTER (WHERE ((a.status)::text = ANY ((ARRAY['In Stock'::character varying, 'Available'::character varying, 'Ready for Deployment'::character varying])::text[]))) AS available_breakdown
...
CREATE TABLE IF NOT EXISTS public.badge_retrieval_assignments (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    offboarding_request_id uuid NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.break_glass_incidents (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    organization_id uuid NOT NULL,
    credential_id uuid NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.bulk_access_operations (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    organization_id uuid NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.certificate_alerts (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    organization_id uuid NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.chat_ticket_conversions (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    session_id uuid NOT NULL,
    ticket_id uuid NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.contact_credentials (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    contact_id uuid NOT NULL,
    credential_id uuid NOT NULL,
    is_primary boolean DEFAULT false,
...
CREATE TABLE IF NOT EXISTS public.contacts (
    ...
    embedding vector,
    embedding_status character varying(20) DEFAULT 'pending',
    embedding_model character varying(100),
...
CREATE TABLE IF NOT EXISTS public.contextual_help (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.credential_access_checkins (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    access_request_id uuid NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.credential_contributions (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    organization_id uuid NOT NULL,
    credential_id uuid,
    action character varying(20) NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.credential_links (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    credential_id uuid NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.credential_sharing_policies (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    credential_id uuid NOT NULL,
    provider_api_key_id uuid,
...
CREATE TABLE IF NOT EXISTS public.credential_tags (
    credential_id uuid NOT NULL,
    tag_id uuid NOT NULL
);
...
CREATE TABLE IF NOT EXISTS public.delegation_transfers (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    organization_id uuid NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.dns_change_alerts (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    organization_id uuid NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.dns_change_windows (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    organization_id uuid NOT NULL,
    domain_id uuid,
    title character varying(255) NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.dns_monitoring_schedules (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    organization_id uuid NOT NULL,
    domain_id uuid NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.dns_monitoring_snapshots (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    domain_id uuid NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.dynamic_group_members (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    group_id uuid NOT NULL,
    contact_id uuid,
    user_id uuid,
...
CREATE TABLE IF NOT EXISTS public.dynamic_group_rules (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    group_id uuid NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.dynamic_groups (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    organization_id uuid NOT NULL,
    name character varying(255) NOT NULL,
    description text,
...
CREATE TABLE IF NOT EXISTS public.interface_links (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.job_title_entitlements (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.kb_article_acknowledgments (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.kb_articles (
    ...
    visible_to_departments character varying(100)[] DEFAULT '{}'::character varying[],
    visible_to_job_titles character varying(100)[] DEFAULT '{}'::character varying[],
    last_modified_by_provider_id uuid,
...
CREATE TABLE IF NOT EXISTS public.kb_contributors (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    organization_id uuid NOT NULL,
    user_id uuid,
    contact_id uuid,
...
CREATE TABLE IF NOT EXISTS public.kb_permissions (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    organization_id uuid NOT NULL,
    user_id uuid,
    contact_id uuid,
...
CREATE TABLE IF NOT EXISTS public.master_data_settings (
    organization_id uuid NOT NULL,
...
CREATE VIEW public.my_project_tasks AS
 SELECT t.id,
  ...
  WHERE ((p.archived_at IS NULL) AND ((t.status)::text <> ALL ((ARRAY['done'::character varying, 'cancelled'::character varying])::text[])))
...
CREATE SEQUENCE IF NOT EXISTS public.offboarding_requests_request_number_seq
    AS integer
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
...
CREATE TABLE IF NOT EXISTS public.offboarding_task_items (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.onboarding_wizard (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    organization_id uuid NOT NULL,
...
CREATE VIEW public.overdue_project_tasks AS
 SELECT t.id,
   ...
   FROM (public.project_tasks t
     JOIN public.projects p ON ((t.project_id = p.id)))
  WHERE (((t.status)::text <> ALL ((ARRAY['done'::character varying, 'cancelled'::character varying])::text[])) AND (t.due_date < CURRENT_DATE))
  ORDER BY t.due_date;


--
-- Name: pending_access_requests; Type: VIEW; Schema: public; Owner: -
--
...
CREATE VIEW public.pending_access_requests AS
 SELECT ar.id,
    ...
    (((for_contact.first_name)::text || ' '::text) || (for_contact.last_name)::text) AS for_contact_name,
    for_contact.department,
    for_contact.job_title,
    (((manager.first_name)::text || ' '::text) || (manager.last_name)::text) AS manager_name,
    manager.email AS manager_email
   FROM (((public.access_requests ar
     JOIN public.contacts requester ON ((ar.requester_id = requester.id)))
     JOIN public.contacts for_contact ON ((ar.for_contact_id = for_contact.id)))
     LEFT JOIN public.contacts manager ON ((for_contact.reports_to_id = manager.id)))
  WHERE ((ar.status)::text = 'pending'::text);


--
-- Name: pending_asset_requests; Type: VIEW; Schema: public; Owner: -
--

CREATE VIEW public.pending_asset_requests AS
 SELECT ar.id,
    ar.organization_id,
    ar.request_number,
    ar.requester_id,
    ...
    ar.updated_at,
    t.name AS tier_name,
    t.category AS tier_category,
    (((requester.first_name)::text || ' '::text) || (requester.last_name)::text) AS requester_name,
    requester.email AS requester_email,
    requester.department AS requester_department,
    (((for_contact.first_name)::text || ' '::text) || (for_contact.last_name)::text) AS for_contact_name,
    for_contact.department AS for_contact_department,
    for_contact.job_title AS for_contact_job_title,
    (((manager.first_name)::text || ' '::text) || (manager.last_name)::text) AS manager_name,
    manager.email AS manager_email
   FROM ((((public.asset_requests ar
     JOIN public.asset_request_tiers t ON ((ar.tier_id = t.id)))
     JOIN public.contacts requester ON ((ar.requester_id = requester.id)))
     JOIN public.contacts for_contact ON ((ar.for_contact_id = for_contact.id)))
     LEFT JOIN public.contacts manager ON ((for_contact.reports_to_id = manager.id)))
  WHERE ((ar.status)::text = 'pending'::text);
...
CREATE TABLE IF NOT EXISTS public.provider_action_log (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    organization_id uuid NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.provider_contract_acknowledgments (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.provider_contributions (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    organization_id uuid NOT NULL,
    provider_api_key_id uuid NOT NULL,
    provider_name character varying(200),
...
CREATE TABLE IF NOT EXISTS public.recurring_ticket_assets (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.recurring_ticket_instances (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    recurring_ticket_id uuid NOT NULL,
    ticket_id uuid NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.recurring_tickets (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    organization_id uuid NOT NULL,
    template_id uuid,
...
CREATE TABLE IF NOT EXISTS public.software_contacts (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    software_id uuid NOT NULL,
    contact_id uuid NOT NULL,
    assigned_at timestamp with time zone DEFAULT now(),
...
CREATE TABLE IF NOT EXISTS public.team_backlog_items (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    organization_id uuid NOT NULL,
    team_id uuid,
    title character varying(255) NOT NULL,
    ...
    CONSTRAINT chk_backlog_status CHECK (((status)::text = ANY ((ARRAY['backlog'::character varying, 'planning'::character varying, 'converted'::character varying, 'done'::character varying, 'aborted'::character varying])::text[])))
);
...
CREATE TABLE IF NOT EXISTS public.ticket_closure_rules (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    organization_id uuid NOT NULL,
    name character varying(255) NOT NULL,
    description text,
...
CREATE TABLE IF NOT EXISTS public.ticket_queue_scores (
    ticket_id uuid NOT NULL,
    organization_id uuid NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.ticket_status_templates (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    name character varying(100) NOT NULL,
...
CREATE VIEW public.user_accessory_return_rates AS
 SELECT ua.organization_id,
    ...
    count(*) FILTER (WHERE ((ua.status)::text <> ALL ((ARRAY['active'::character varying, 'returned'::character varying, 'transferred'::character varying])::text[]))) AS total_not_returned,
        CASE
    ...
    COALESCE(sum((a.unit_cost * ((ua.quantity - COALESCE(ua.returned_quantity, 0)))::numeric)) FILTER (WHERE ((ua.status)::text <> ALL ((ARRAY['active'::character varying, 'returned'::character varying, 'transferred'::character varying])::text[]))), (0)::numeric) AS unreturned_value
   FROM ((public.user_assignments ua
...
-- vendor_contacts and vendors tables removed in migration 053
-- Vendors are now stored in companies table with type='vendor'
-- Vendor contacts are stored in contacts table with contact_type='vendor'
...
--
-- Name: client_tag_assignments company_tag_assignments_company_id_tag_id_key; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: client_tags client_tags_organization_id_name_key; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: ticket_categories ticket_categories_organization_id_name_key; Type: CONSTRAINT; Schema: public; Owner: -
--
...
-- vendor_contacts_pkey and vendors_pkey removed (tables dropped in migration 053)
...
CREATE INDEX IF NOT EXISTS idx_telemetry_log_org ON public.telemetry_log USING btree (organization_id, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_telemetry_log_status ON public.telemetry_log USING btree (status) WHERE (status = 'pending'::character varying);
...
CREATE INDEX IF NOT EXISTS idx_access_profile_items_profile ON public.access_profile_items USING btree (profile_id);
...
CREATE INDEX IF NOT EXISTS idx_access_profiles_org ON public.access_profiles USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_access_requests_approver ON public.access_requests USING btree (approver_id);
...
CREATE INDEX IF NOT EXISTS idx_access_requests_org ON public.access_requests USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_access_requests_requester ON public.access_requests USING btree (requester_id);
...
CREATE INDEX IF NOT EXISTS idx_accessories_category ON public.accessories USING btree (category);
...
CREATE INDEX IF NOT EXISTS idx_accessories_organization ON public.accessories USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_accessory_disposals_accessory ON public.accessory_disposals USING btree (accessory_id);
...
CREATE INDEX IF NOT EXISTS idx_accessory_disposals_org ON public.accessory_disposals USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_accessory_disposals_type ON public.accessory_disposals USING btree (disposal_type);
...
CREATE INDEX IF NOT EXISTS idx_accessory_inventory_transactions_accessory ON public.accessory_inventory_transactions USING btree (accessory_id);
...
CREATE INDEX IF NOT EXISTS idx_accessory_models_org ON public.accessory_models USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_accessory_purchase_order_items_po ON public.accessory_purchase_order_items USING btree (purchase_order_id);
...
CREATE INDEX IF NOT EXISTS idx_accessory_purchase_orders_org ON public.accessory_purchase_orders USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_accessory_purchase_orders_vendor ON public.accessory_purchase_orders USING btree (vendor_id);
...
CREATE INDEX IF NOT EXISTS idx_account_user_id ON public.account USING btree ("userId");
...
CREATE INDEX IF NOT EXISTS idx_ai_action_executions_session ON public.ai_action_executions USING btree (session_id);
...
CREATE INDEX IF NOT EXISTS idx_ai_actions_org ON public.ai_actions USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_ai_category_matches_ticket ON public.ai_category_matches USING btree (ticket_id);
...
CREATE INDEX IF NOT EXISTS idx_ai_category_matches_workspace ON public.ai_category_matches USING btree (workspace_id);
...
CREATE INDEX IF NOT EXISTS idx_ai_chat_messages_session ON public.ai_chat_messages USING btree (session_id);
...
CREATE INDEX IF NOT EXISTS idx_ai_chat_sessions_contact ON public.ai_chat_sessions USING btree (contact_id);
...
CREATE INDEX IF NOT EXISTS idx_ai_chat_sessions_context ON public.ai_chat_sessions USING btree (context_level);
...
CREATE INDEX IF NOT EXISTS idx_ai_chat_sessions_organization ON public.ai_chat_sessions USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_ai_chat_sessions_resolution ON public.ai_chat_sessions USING btree (resolution_status);
...
CREATE INDEX IF NOT EXISTS idx_ai_chat_sessions_support ON public.ai_chat_sessions USING btree (is_support_chat);
...
CREATE INDEX IF NOT EXISTS idx_ai_chat_sessions_type ON public.ai_chat_sessions USING btree (chat_type);
...
CREATE INDEX IF NOT EXISTS idx_ai_chat_sessions_user ON public.ai_chat_sessions USING btree (user_id);
...
CREATE INDEX IF NOT EXISTS idx_ai_data_access_log_created ON public.ai_chat_data_access_log USING btree (created_at);
...
CREATE INDEX IF NOT EXISTS idx_ai_data_access_log_resource ON public.ai_chat_data_access_log USING btree (resource_type, resource_id);
...
CREATE INDEX IF NOT EXISTS idx_ai_data_access_log_session ON public.ai_chat_data_access_log USING btree (session_id);
...
CREATE INDEX IF NOT EXISTS idx_ai_data_access_policies_level ON public.ai_data_access_policies USING btree (context_level);
...
CREATE INDEX IF NOT EXISTS idx_ai_data_access_policies_org ON public.ai_data_access_policies USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_ai_triage_results_ticket ON public.ai_triage_results USING btree (ticket_id);
...
CREATE INDEX IF NOT EXISTS idx_ai_triage_results_workspace ON public.ai_triage_results USING btree (workspace_id);
...
CREATE INDEX IF NOT EXISTS idx_alert_correlation_rules_org ON public.alert_correlation_rules USING btree (organization_id, is_active);
...
CREATE INDEX IF NOT EXISTS idx_alert_suppression_rules_org ON public.alert_suppression_rules USING btree (organization_id, is_active);
...
CREATE INDEX IF NOT EXISTS idx_alerts_asset ON public.alerts USING btree (asset_id);
...
CREATE INDEX IF NOT EXISTS idx_alerts_correlation ON public.alerts USING btree (correlation_key);
...
CREATE INDEX IF NOT EXISTS idx_alerts_fingerprint ON public.alerts USING btree (fingerprint);
...
CREATE INDEX IF NOT EXISTS idx_alerts_fired_at ON public.alerts USING btree (fired_at);
...
CREATE INDEX IF NOT EXISTS idx_alerts_incident ON public.alerts USING btree (incident_id);
...
CREATE INDEX IF NOT EXISTS idx_alerts_organization ON public.alerts USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_alerts_source ON public.alerts USING btree (source, source_alert_id);
...
CREATE INDEX IF NOT EXISTS idx_approval_workflow_steps_workflow ON public.approval_workflow_steps USING btree (workflow_id);
...
CREATE INDEX IF NOT EXISTS idx_approval_workflows_org ON public.approval_workflows USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_asset_contacts_asset ON public.asset_contacts USING btree (asset_id);
...
CREATE INDEX IF NOT EXISTS idx_asset_contacts_contact ON public.asset_contacts USING btree (contact_id);
...
CREATE INDEX IF NOT EXISTS idx_asset_credentials_asset ON public.asset_credentials USING btree (asset_id);
...
CREATE INDEX IF NOT EXISTS idx_asset_credentials_credential ON public.asset_credentials USING btree (credential_id);
...
CREATE INDEX IF NOT EXISTS idx_asset_documents_document ON public.asset_documents USING btree (document_id);
...
CREATE INDEX IF NOT EXISTS idx_asset_history_asset ON public.asset_history USING btree (asset_id);
...
CREATE INDEX IF NOT EXISTS idx_asset_import_items_job ON public.asset_import_items USING btree (import_job_id);
...
CREATE INDEX IF NOT EXISTS idx_asset_import_jobs_org ON public.asset_import_jobs USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_asset_import_jobs_status ON public.asset_import_jobs USING btree (status);
...
CREATE INDEX IF NOT EXISTS idx_asset_interfaces_asset ON public.asset_interfaces USING btree (asset_id);
...
CREATE INDEX IF NOT EXISTS idx_asset_models_org ON public.asset_models USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_asset_models_subtype ON public.asset_models USING btree (asset_subtype_id);
...
CREATE INDEX IF NOT EXISTS idx_asset_models_vendor ON public.asset_models USING btree (vendor_id);
...
CREATE INDEX IF NOT EXISTS idx_asset_naming_templates_org ON public.asset_naming_templates USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_asset_request_tiers_category ON public.asset_request_tiers USING btree (category);
...
CREATE INDEX IF NOT EXISTS idx_asset_request_tiers_org ON public.asset_request_tiers USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_asset_requests_org ON public.asset_requests USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_asset_requests_requester ON public.asset_requests USING btree (requester_id);
...
CREATE INDEX IF NOT EXISTS idx_asset_requests_tier ON public.asset_requests USING btree (tier_id);
...
CREATE INDEX IF NOT EXISTS idx_asset_retrieval_request ON public.asset_retrieval_assignments USING btree (offboarding_request_id);
...
CREATE INDEX IF NOT EXISTS idx_asset_retrieval_status ON public.asset_retrieval_assignments USING btree (status);
...
CREATE INDEX IF NOT EXISTS idx_asset_software_asset ON public.asset_software USING btree (asset_id);
...
CREATE INDEX IF NOT EXISTS idx_asset_software_software ON public.asset_software USING btree (software_id);
...
CREATE INDEX IF NOT EXISTS idx_asset_subtypes_active ON public.asset_subtypes USING btree (organization_id, is_active) WHERE (is_active = true);
...
CREATE INDEX IF NOT EXISTS idx_asset_subtypes_org ON public.asset_subtypes USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_asset_subtypes_type ON public.asset_subtypes USING btree (asset_type_id);
...
CREATE INDEX IF NOT EXISTS idx_asset_tickets_asset ON public.asset_tickets USING btree (asset_id);
...
CREATE INDEX IF NOT EXISTS idx_asset_tickets_ticket ON public.asset_tickets USING btree (ticket_id);
...
CREATE INDEX IF NOT EXISTS idx_assets_company ON public.assets USING btree (company_id);
...
CREATE INDEX IF NOT EXISTS idx_assets_deleted ON public.assets USING btree (is_deleted) WHERE (is_deleted = true);
...
CREATE INDEX IF NOT EXISTS idx_assets_external ON public.assets USING btree (external_source, external_id) WHERE (external_id IS NOT NULL);
...
CREATE INDEX IF NOT EXISTS idx_assets_model ON public.assets USING btree (model_id);
...
CREATE INDEX IF NOT EXISTS idx_assets_os ON public.assets USING btree (os_id);
...
CREATE INDEX IF NOT EXISTS idx_assets_requestable ON public.assets USING btree (is_requestable) WHERE (is_requestable = true);
...
CREATE INDEX IF NOT EXISTS idx_assets_serial ON public.assets USING btree (serial_number);
...
CREATE INDEX IF NOT EXISTS idx_assets_subtype ON public.assets USING btree (subtype_id);
...
CREATE INDEX IF NOT EXISTS idx_assets_tier ON public.assets USING btree (request_tier_id);
...
CREATE INDEX IF NOT EXISTS idx_assets_type ON public.assets USING btree (type_id);
...
CREATE INDEX IF NOT EXISTS idx_backlog_comments ON public.backlog_comments USING btree (backlog_item_id);
...
CREATE INDEX IF NOT EXISTS idx_backlog_org ON public.team_backlog_items USING btree (organization_id, status);
...
CREATE INDEX IF NOT EXISTS idx_backlog_review ON public.team_backlog_items USING btree (review_by) WHERE ((status)::text = ANY ((ARRAY['backlog'::character varying, 'planning'::character varying])::text[]));
...
CREATE INDEX IF NOT EXISTS idx_backlog_team ON public.team_backlog_items USING btree (team_id, status);
...
CREATE INDEX IF NOT EXISTS idx_break_glass_incidents_org ON public.break_glass_incidents USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_break_glass_incidents_status ON public.break_glass_incidents USING btree (status);
...
CREATE INDEX IF NOT EXISTS idx_catalog_categories_org ON public.catalog_categories USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_catalog_categories_parent ON public.catalog_categories USING btree (parent_id);
...
CREATE INDEX IF NOT EXISTS idx_catalog_items_active ON public.catalog_items USING btree (is_active, is_requestable);
...
CREATE INDEX IF NOT EXISTS idx_catalog_items_category ON public.catalog_items USING btree (category_id);
...
CREATE INDEX IF NOT EXISTS idx_catalog_items_type ON public.catalog_items USING btree (item_type);
...
CREATE INDEX IF NOT EXISTS idx_category_requests_pending ON public.category_requests USING btree (workspace_id) WHERE ((status)::text = 'pending'::text);
...
CREATE INDEX IF NOT EXISTS idx_category_requests_workspace ON public.category_requests USING btree (workspace_id);
...
CREATE INDEX IF NOT EXISTS idx_certificate_alerts_cert ON public.certificate_alerts USING btree (certificate_id);
...
CREATE INDEX IF NOT EXISTS idx_certificate_alerts_status ON public.certificate_alerts USING btree (status);
...
CREATE INDEX IF NOT EXISTS idx_certificate_assets_asset ON public.certificate_assets USING btree (asset_id);
...
CREATE INDEX IF NOT EXISTS idx_certificate_assets_cert ON public.certificate_assets USING btree (certificate_id);
...
CREATE INDEX IF NOT EXISTS idx_certificate_history_cert ON public.certificate_history USING btree (certificate_id);
...
CREATE INDEX IF NOT EXISTS idx_certificates_domain ON public.certificates USING btree (domain_id);
...
CREATE INDEX IF NOT EXISTS idx_certificates_org ON public.certificates USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_certificates_organization ON public.certificates USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_chat_handoffs_assigned ON public.chat_handoffs USING btree (assigned_to);
...
CREATE INDEX IF NOT EXISTS idx_chat_handoffs_session ON public.chat_handoffs USING btree (session_id);
...
CREATE INDEX IF NOT EXISTS idx_chat_handoffs_status ON public.chat_handoffs USING btree (status);
...
CREATE INDEX IF NOT EXISTS idx_chat_quick_replies_org ON public.chat_quick_replies USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_chat_quick_replies_shortcut ON public.chat_quick_replies USING btree (shortcut);
...
CREATE INDEX IF NOT EXISTS idx_chat_ticket_conversions_session ON public.chat_ticket_conversions USING btree (session_id);
...
CREATE INDEX IF NOT EXISTS idx_chat_ticket_conversions_ticket ON public.chat_ticket_conversions USING btree (ticket_id);
...
CREATE INDEX IF NOT EXISTS idx_checklist_template_items ON public.checklist_template_items USING btree (template_id);
...
CREATE INDEX IF NOT EXISTS idx_checklist_templates_category ON public.checklist_templates USING btree (category_id) WHERE (is_active = true);
...
CREATE INDEX IF NOT EXISTS idx_clients_external ON public.companies USING btree (external_source, external_id) WHERE (external_id IS NOT NULL);
...
CREATE INDEX IF NOT EXISTS idx_clients_name ON public.companies USING btree (name);
...
CREATE INDEX IF NOT EXISTS idx_clients_organization ON public.companies USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_contact_credentials_contact ON public.contact_credentials USING btree (contact_id);
...
CREATE INDEX IF NOT EXISTS idx_contact_credentials_credential ON public.contact_credentials USING btree (credential_id);
...
CREATE INDEX IF NOT EXISTS idx_contacts_deleted ON public.contacts USING btree (is_deleted) WHERE (is_deleted = true);


--
-- Name: idx_contacts_department; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX IF NOT EXISTS idx_contacts_department ON public.contacts USING btree (department);
...
CREATE INDEX IF NOT EXISTS idx_contacts_department_new ON public.contacts USING btree (department_id);
...
CREATE INDEX IF NOT EXISTS idx_contacts_external ON public.contacts USING btree (external_source, external_id) WHERE (external_id IS NOT NULL);
...
CREATE INDEX IF NOT EXISTS idx_contacts_job_title ON public.contacts USING btree (job_title_id);
...
CREATE INDEX IF NOT EXISTS idx_contacts_location ON public.contacts USING btree (location_id);
...
CREATE INDEX IF NOT EXISTS idx_contacts_organization ON public.contacts USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_contacts_embedding_status ON public.contacts (embedding_status) WHERE embedding_status != 'complete';
...
CREATE INDEX IF NOT EXISTS idx_contacts_reports_to ON public.contacts USING btree (reports_to_id);
...
CREATE INDEX IF NOT EXISTS idx_contextual_help_feature ON public.contextual_help USING btree (feature_key, context);
...
CREATE INDEX IF NOT EXISTS idx_contracts_organization ON public.contracts USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_cred_access_requests_cred ON public.credential_access_requests USING btree (credential_id);
...
CREATE INDEX IF NOT EXISTS idx_cred_access_requests_key ON public.credential_access_requests USING btree (provider_api_key_id);
...
CREATE INDEX IF NOT EXISTS idx_cred_access_requests_status ON public.credential_access_requests USING btree (status, access_expires_at);
...
CREATE INDEX IF NOT EXISTS idx_cred_reveal_log_break_glass ON public.credential_reveal_log USING btree (is_break_glass) WHERE (is_break_glass = true);
...
CREATE INDEX IF NOT EXISTS idx_cred_reveal_log_cred ON public.credential_reveal_log USING btree (credential_id);
...
CREATE INDEX IF NOT EXISTS idx_cred_reveal_log_time ON public.credential_reveal_log USING btree (revealed_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_cred_sharing_policies_cred ON public.credential_sharing_policies USING btree (credential_id);
...
CREATE INDEX IF NOT EXISTS idx_cred_sharing_policies_key ON public.credential_sharing_policies USING btree (provider_api_key_id);
...
CREATE INDEX IF NOT EXISTS idx_credential_access_credential ON public.credential_access USING btree (credential_id);
...
CREATE INDEX IF NOT EXISTS idx_credential_access_log_credential ON public.credential_access_log USING btree (credential_id);
...
CREATE INDEX IF NOT EXISTS idx_credential_access_user ON public.credential_access USING btree (user_id);
...
CREATE INDEX IF NOT EXISTS idx_credential_contributions_ack ON public.credential_contributions USING btree (client_acknowledged) WHERE (client_acknowledged = false);
...
CREATE INDEX IF NOT EXISTS idx_credential_contributions_cred ON public.credential_contributions USING btree (credential_id);
...
CREATE INDEX IF NOT EXISTS idx_credential_contributions_org ON public.credential_contributions USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_credential_links_credential ON public.credential_links USING btree (credential_id);
...
CREATE INDEX IF NOT EXISTS idx_credential_links_target ON public.credential_links USING btree (link_type, link_id);
...
CREATE INDEX IF NOT EXISTS idx_credentials_company ON public.credentials USING btree (company_id);
...
CREATE INDEX IF NOT EXISTS idx_credentials_deleted ON public.credentials USING btree (is_deleted) WHERE (is_deleted = true);
...
CREATE INDEX IF NOT EXISTS idx_credentials_name ON public.credentials USING btree (name);
...
CREATE INDEX IF NOT EXISTS idx_credentials_organization ON public.credentials USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_dashboard_configs_org ON public.dashboard_configs USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_dashboard_configs_type ON public.dashboard_configs USING btree (view_type);
...
CREATE INDEX IF NOT EXISTS idx_delegation_rules_delegator ON public.delegation_rules USING btree (delegator_type, delegator_id);
...
CREATE INDEX IF NOT EXISTS idx_delegation_rules_org ON public.delegation_rules USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_delegation_transfers_dates ON public.delegation_transfers USING btree (starts_at, ends_at);
...
CREATE INDEX IF NOT EXISTS idx_delegation_transfers_delegate ON public.delegation_transfers USING btree (delegate_id);
...
CREATE INDEX IF NOT EXISTS idx_delegation_transfers_delegator ON public.delegation_transfers USING btree (delegator_id);
...
CREATE INDEX IF NOT EXISTS idx_departments_org ON public.departments USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_departments_path ON public.departments USING btree (path);
...
CREATE INDEX IF NOT EXISTS idx_dns_change_alerts_domain ON public.dns_change_alerts USING btree (domain_id);
...
CREATE INDEX IF NOT EXISTS idx_dns_change_alerts_org ON public.dns_change_alerts USING btree (organization_id, status);
...
CREATE INDEX IF NOT EXISTS idx_dns_change_alerts_status ON public.dns_change_alerts USING btree (status);
...
CREATE INDEX IF NOT EXISTS idx_dns_change_windows_dates ON public.dns_change_windows USING btree (starts_at, ends_at);
...
CREATE INDEX IF NOT EXISTS idx_dns_change_windows_domain ON public.dns_change_windows USING btree (domain_id);
...
CREATE INDEX IF NOT EXISTS idx_dns_monitoring_schedules_domain ON public.dns_monitoring_schedules USING btree (domain_id);
...
CREATE INDEX IF NOT EXISTS idx_dns_monitoring_schedules_next ON public.dns_monitoring_schedules USING btree (next_check_at, is_active);
...
CREATE INDEX IF NOT EXISTS idx_dns_monitoring_snapshots_checked ON public.dns_monitoring_snapshots USING btree (checked_at);
...
CREATE INDEX IF NOT EXISTS idx_dns_monitoring_snapshots_domain ON public.dns_monitoring_snapshots USING btree (domain_id);
...
CREATE INDEX IF NOT EXISTS idx_dns_records_domain ON public.dns_records USING btree (domain_id);
...
CREATE INDEX IF NOT EXISTS idx_documents_company ON public.documents USING btree (company_id);
...
CREATE INDEX IF NOT EXISTS idx_documents_deleted ON public.documents USING btree (is_deleted) WHERE (is_deleted = true);
...
CREATE INDEX IF NOT EXISTS idx_documents_organization ON public.documents USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_documents_search ON public.documents USING gin (to_tsvector('english'::regconfig, (((COALESCE(title, ''::character varying))::text || ' '::text) || COALESCE(content_raw, ''::text))));
...
CREATE INDEX IF NOT EXISTS idx_documents_title ON public.documents USING btree (title);
...
CREATE INDEX IF NOT EXISTS idx_domains_company ON public.domains USING btree (company_id);
...
CREATE INDEX IF NOT EXISTS idx_domains_expires ON public.domains USING btree (expires_at);
...
CREATE INDEX IF NOT EXISTS idx_domains_organization ON public.domains USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_dynamic_group_members_asset ON public.dynamic_group_members USING btree (asset_id) WHERE (is_active = true);
...
CREATE INDEX IF NOT EXISTS idx_dynamic_group_members_contact ON public.dynamic_group_members USING btree (contact_id) WHERE (is_active = true);
...
CREATE INDEX IF NOT EXISTS idx_dynamic_group_members_group ON public.dynamic_group_members USING btree (group_id) WHERE (is_active = true);
...
CREATE INDEX IF NOT EXISTS idx_dynamic_group_members_user ON public.dynamic_group_members USING btree (user_id) WHERE (is_active = true);
...
CREATE INDEX IF NOT EXISTS idx_dynamic_group_rules_group ON public.dynamic_group_rules USING btree (group_id);
...
CREATE INDEX IF NOT EXISTS idx_dynamic_groups_org ON public.dynamic_groups USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_dynamic_groups_refresh ON public.dynamic_groups USING btree (last_evaluated_at) WHERE (((membership_type)::text = 'dynamic'::text) AND (refresh_interval_minutes > 0));
...
CREATE INDEX IF NOT EXISTS idx_dynamic_groups_type ON public.dynamic_groups USING btree (group_type);
...
CREATE INDEX IF NOT EXISTS idx_email_delivery_logs_queue ON public.email_delivery_logs USING btree (email_queue_id);
...
CREATE INDEX IF NOT EXISTS idx_email_events_email ON public.email_events USING btree (email_id);
...
CREATE INDEX IF NOT EXISTS idx_email_preferences_contact ON public.email_preferences USING btree (contact_id);
...
CREATE INDEX IF NOT EXISTS idx_email_preferences_user ON public.email_preferences USING btree (user_id);
...
CREATE INDEX IF NOT EXISTS idx_email_queue_created ON public.email_queue USING btree (created_at);
...
CREATE INDEX IF NOT EXISTS idx_email_queue_pending ON public.email_queue USING btree (status, scheduled_at) WHERE ((status)::text = 'pending'::text);
...
CREATE INDEX IF NOT EXISTS idx_email_queue_retry ON public.email_queue USING btree (status, next_retry_at) WHERE ((status)::text = 'failed'::text);
...
CREATE INDEX IF NOT EXISTS idx_employment_types_org ON public.employment_types USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_feature_registry_category ON public.feature_registry USING btree (category);
...
CREATE INDEX IF NOT EXISTS idx_feature_registry_status ON public.feature_registry USING btree (status);
...
CREATE INDEX IF NOT EXISTS idx_feature_usage_log_created ON public.feature_usage_log USING btree (created_at);
...
CREATE INDEX IF NOT EXISTS idx_feature_usage_log_feature ON public.feature_usage_log USING btree (feature_key);
...
CREATE INDEX IF NOT EXISTS idx_feature_usage_log_org ON public.feature_usage_log USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_files_reference ON public.files USING btree (reference_type, reference_id);
...
CREATE INDEX IF NOT EXISTS idx_folders_organization ON public.folders USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_folders_parent ON public.folders USING btree (parent_id);
...
CREATE INDEX IF NOT EXISTS idx_incident_timeline_incident ON public.incident_timeline USING btree (incident_id);
...
CREATE INDEX IF NOT EXISTS idx_incidents_correlation ON public.incidents USING btree (correlation_key);
...
CREATE INDEX IF NOT EXISTS idx_incidents_organization ON public.incidents USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_incidents_ticket ON public.incidents USING btree (ticket_id);
...
CREATE INDEX IF NOT EXISTS idx_job_title_entitlements_org ON public.job_title_entitlements USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_job_title_entitlements_title ON public.job_title_entitlements USING btree (job_title_id);
...
CREATE INDEX IF NOT EXISTS idx_job_titles_active ON public.job_titles USING btree (organization_id, is_active);
...
CREATE INDEX IF NOT EXISTS idx_job_titles_cloned ON public.job_titles USING btree (cloned_from_id);
...
CREATE INDEX IF NOT EXISTS idx_job_titles_org ON public.job_titles USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_kb_article_ack_article ON public.kb_article_acknowledgments USING btree (article_id);
...
CREATE INDEX IF NOT EXISTS idx_kb_article_ack_user ON public.kb_article_acknowledgments USING btree (user_id);
...
CREATE INDEX IF NOT EXISTS idx_kb_article_versions_article ON public.kb_article_versions USING btree (article_id);
...
CREATE INDEX IF NOT EXISTS idx_kb_articles_org ON public.kb_articles USING btree (organization_id);
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_kb_articles_slug ON public.kb_articles USING btree (organization_id, slug);
...
CREATE INDEX IF NOT EXISTS idx_kb_articles_status ON public.kb_articles USING btree (organization_id, status);
...
CREATE INDEX IF NOT EXISTS idx_kb_articles_system ON public.kb_articles (organization_id, is_system) WHERE is_system = true;
...
CREATE INDEX IF NOT EXISTS idx_kb_articles_embedding_status ON public.kb_articles (embedding_status) WHERE embedding_status != 'complete';
...
CREATE INDEX IF NOT EXISTS idx_kb_article_chunks_article ON public.kb_article_chunks USING btree (article_id);
...
CREATE INDEX IF NOT EXISTS idx_kb_article_chunks_org ON public.kb_article_chunks USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_kb_article_chunks_embedding_status ON public.kb_article_chunks (embedding_status) WHERE embedding_status != 'complete';
...
ALTER TABLE ONLY public.kb_article_chunks DROP CONSTRAINT IF EXISTS kb_article_chunks_pkey;
ALTER TABLE ONLY public.kb_article_chunks ADD CONSTRAINT kb_article_chunks_pkey PRIMARY KEY (id);
...
ALTER TABLE ONLY public.kb_article_chunks DROP CONSTRAINT IF EXISTS kb_article_chunks_article_chunk_unique;
ALTER TABLE ONLY public.kb_article_chunks ADD CONSTRAINT kb_article_chunks_article_chunk_unique UNIQUE (article_id, chunk_index);
...
ALTER TABLE ONLY public.kb_article_chunks DROP CONSTRAINT IF EXISTS kb_article_chunks_article_id_fkey;
ALTER TABLE ONLY public.kb_article_chunks ADD CONSTRAINT kb_article_chunks_article_id_fkey FOREIGN KEY (article_id) REFERENCES public.kb_articles(id) ON DELETE CASCADE;
...
CREATE INDEX IF NOT EXISTS idx_kb_categories_org ON public.kb_categories USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_kb_categories_slug ON public.kb_categories USING btree (slug);
...
CREATE INDEX IF NOT EXISTS idx_kb_contributors_contact ON public.kb_contributors USING btree (contact_id);
...
CREATE INDEX IF NOT EXISTS idx_kb_contributors_user ON public.kb_contributors USING btree (user_id);
...
CREATE INDEX IF NOT EXISTS idx_kb_permissions_perm ON public.kb_permissions USING btree (permission);
...
CREATE INDEX IF NOT EXISTS idx_kb_permissions_role ON public.kb_permissions USING btree (role_id);
...
CREATE INDEX IF NOT EXISTS idx_kb_permissions_user ON public.kb_permissions USING btree (user_id);
...
CREATE INDEX IF NOT EXISTS idx_locations_company ON public.locations USING btree (company_id);
...
CREATE INDEX IF NOT EXISTS idx_maintenance_history_schedule ON public.maintenance_history USING btree (schedule_id);
...
CREATE INDEX IF NOT EXISTS idx_maintenance_schedules_asset ON public.maintenance_schedules USING btree (asset_id);
...
CREATE INDEX IF NOT EXISTS idx_maintenance_schedules_next ON public.maintenance_schedules USING btree (next_maintenance_at);
...
CREATE INDEX IF NOT EXISTS idx_maintenance_schedules_org ON public.maintenance_schedules USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_manager_relationships_manager ON public.manager_relationships USING btree (manager_id);
...
CREATE INDEX IF NOT EXISTS idx_manager_relationships_user ON public.manager_relationships USING btree (user_id);
...
CREATE INDEX IF NOT EXISTS idx_master_data_requests_org ON public.master_data_requests USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_master_data_requests_status ON public.master_data_requests USING btree (status);
...
CREATE INDEX IF NOT EXISTS idx_networks_company ON public.networks USING btree (company_id);
...
CREATE INDEX IF NOT EXISTS idx_networks_organization ON public.networks USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_notification_prefs_user ON public.notification_preferences USING btree (user_id);
...
CREATE INDEX IF NOT EXISTS idx_notification_templates_event ON public.notification_templates USING btree (event_type);
...
CREATE INDEX IF NOT EXISTS idx_notification_templates_org ON public.notification_templates USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_offboarding_requests_org ON public.offboarding_requests USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_offboarding_requests_status ON public.offboarding_requests USING btree (status);
...
CREATE INDEX IF NOT EXISTS idx_offboarding_task_items_offboarding ON public.offboarding_task_items USING btree (offboarding_id);
...
CREATE INDEX IF NOT EXISTS idx_offboarding_task_items_status ON public.offboarding_task_items USING btree (status);
...
CREATE INDEX IF NOT EXISTS idx_offboarding_tasks_organization ON public.offboarding_tasks USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_offboarding_tasks_user ON public.offboarding_tasks USING btree (user_id);
...
CREATE INDEX IF NOT EXISTS idx_office_locations_coords ON public.office_locations USING btree (latitude, longitude) WHERE (latitude IS NOT NULL);
...
CREATE INDEX IF NOT EXISTS idx_office_locations_level ON public.office_locations USING btree (location_level);
...
CREATE INDEX IF NOT EXISTS idx_office_locations_org ON public.office_locations USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_office_locations_parent ON public.office_locations USING btree (parent_id);
...
CREATE INDEX IF NOT EXISTS idx_office_locations_path ON public.office_locations USING btree (path);
...
CREATE INDEX IF NOT EXISTS idx_onboarding_requests_org ON public.onboarding_requests USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_onboarding_requests_status ON public.onboarding_requests USING btree (organization_id, status);
...
CREATE INDEX IF NOT EXISTS idx_onboarding_requests_ticket ON public.onboarding_requests USING btree (ticket_id);
...
CREATE INDEX IF NOT EXISTS idx_onboarding_tasks_request ON public.onboarding_tasks USING btree (onboarding_request_id);
...
CREATE INDEX IF NOT EXISTS idx_operating_systems_org ON public.operating_systems USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_operating_systems_platform ON public.operating_systems USING btree (platform);
...
CREATE INDEX IF NOT EXISTS idx_org_domains_domain ON public.organization_domains USING btree (domain);
...
CREATE INDEX IF NOT EXISTS idx_org_domains_org ON public.organization_domains USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_org_domains_type ON public.organization_domains USING btree (domain_type);
...
CREATE INDEX IF NOT EXISTS idx_org_feature_flags_feature ON public.organization_feature_flags USING btree (feature_key);
...
CREATE INDEX IF NOT EXISTS idx_org_feature_flags_org ON public.organization_feature_flags USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_permission_sets_org ON public.permission_sets USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_project_activity_project ON public.project_activity USING btree (project_id);
...
CREATE INDEX IF NOT EXISTS idx_project_milestones_due ON public.project_milestones USING btree (due_date);
...
CREATE INDEX IF NOT EXISTS idx_project_tasks_assigned ON public.project_tasks USING btree (assigned_to);
...
CREATE INDEX IF NOT EXISTS idx_project_tasks_milestone ON public.project_tasks USING btree (milestone_id);
...
CREATE INDEX IF NOT EXISTS idx_projects_dates ON public.projects USING btree (start_date, target_end_date);
...
CREATE INDEX IF NOT EXISTS idx_projects_manager ON public.projects USING btree (manager_id);
...
CREATE INDEX IF NOT EXISTS idx_provider_access_grants_active ON public.provider_access_grants USING btree (is_active, valid_until);
...
CREATE INDEX IF NOT EXISTS idx_provider_access_grants_provider ON public.provider_access_grants USING btree (provider_id);
...
CREATE INDEX IF NOT EXISTS idx_provider_action_log_key ON public.provider_action_log USING btree (api_key_id);
...
CREATE INDEX IF NOT EXISTS idx_provider_action_log_org ON public.provider_action_log USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_provider_action_log_resource ON public.provider_action_log USING btree (resource_type, resource_id);
...
CREATE INDEX IF NOT EXISTS idx_provider_action_log_time ON public.provider_action_log USING btree (started_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_provider_api_keys_active ON public.provider_api_keys USING btree (is_active, is_revoked);
...
CREATE INDEX IF NOT EXISTS idx_provider_api_keys_hash ON public.provider_api_keys USING btree (key_hash);
...
CREATE INDEX IF NOT EXISTS idx_provider_api_keys_org ON public.provider_api_keys USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_provider_contract_acks_key ON public.provider_contract_acknowledgments USING btree (api_key_id);
...
CREATE INDEX IF NOT EXISTS idx_provider_contracts_org ON public.provider_contracts USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_provider_contributions_entity ON public.provider_contributions USING btree (entity_type, entity_id);
...
CREATE INDEX IF NOT EXISTS idx_provider_contributions_org ON public.provider_contributions USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_provider_contributions_provider ON public.provider_contributions USING btree (provider_api_key_id);
...
CREATE INDEX IF NOT EXISTS idx_provider_contributions_time ON public.provider_contributions USING btree (created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_provider_sessions_active ON public.provider_sessions USING btree (is_active, expires_at);
...
CREATE INDEX IF NOT EXISTS idx_provider_sessions_key ON public.provider_sessions USING btree (api_key_id);
...
CREATE INDEX IF NOT EXISTS idx_provider_users_email ON public.provider_users USING btree (email);
...
CREATE INDEX IF NOT EXISTS idx_provider_users_provider ON public.provider_users USING btree (provider_id);
...
CREATE INDEX IF NOT EXISTS idx_queue_scores_action_state ON public.ticket_queue_scores USING btree (organization_id, action_state);
...
CREATE INDEX IF NOT EXISTS idx_queue_scores_org_score ON public.ticket_queue_scores USING btree (organization_id, base_score DESC);
...
CREATE INDEX IF NOT EXISTS idx_recurring_instances_recurring ON public.recurring_ticket_instances USING btree (recurring_ticket_id);
...
CREATE INDEX IF NOT EXISTS idx_recurring_instances_ticket ON public.recurring_ticket_instances USING btree (ticket_id);
...
CREATE INDEX IF NOT EXISTS idx_recurring_tickets_active ON public.recurring_tickets USING btree (is_active);
...
CREATE INDEX IF NOT EXISTS idx_recurring_tickets_org ON public.recurring_tickets USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_request_activity_request ON public.request_activity USING btree (request_id);
...
CREATE INDEX IF NOT EXISTS idx_request_approvals_approver ON public.request_approvals USING btree (approver_id, decision);
...
CREATE INDEX IF NOT EXISTS idx_request_approvals_request ON public.request_approvals USING btree (request_id);
...
CREATE INDEX IF NOT EXISTS idx_request_subcategories_category ON public.request_subcategories USING btree (category);
...
CREATE INDEX IF NOT EXISTS idx_request_subcategories_org ON public.request_subcategories USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_routing_rules_priority ON public.routing_rules USING btree (workspace_id, priority) WHERE (is_active = true);
...
CREATE INDEX IF NOT EXISTS idx_routing_rules_workspace ON public.routing_rules USING btree (workspace_id);
...
CREATE INDEX IF NOT EXISTS idx_saas_services_category ON public.saas_services USING btree (category);
...
CREATE INDEX IF NOT EXISTS idx_saas_services_organization ON public.saas_services USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_service_owners_available ON public.service_owners USING btree (is_available) WHERE (is_available = true);
...
CREATE INDEX IF NOT EXISTS idx_service_owners_contact ON public.service_owners USING btree (contact_id);
...
CREATE INDEX IF NOT EXISTS idx_service_owners_service ON public.service_owners USING btree (service_id);
...
CREATE INDEX IF NOT EXISTS idx_service_owners_user ON public.service_owners USING btree (user_id);
...
CREATE INDEX IF NOT EXISTS idx_service_providers_org ON public.service_providers USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_service_providers_status ON public.service_providers USING btree (status);
...
CREATE INDEX IF NOT EXISTS idx_service_requests_item ON public.service_requests USING btree (catalog_item_id);
...
CREATE INDEX IF NOT EXISTS idx_service_requests_org ON public.service_requests USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_service_requests_requester ON public.service_requests USING btree (requester_id);
...
CREATE INDEX IF NOT EXISTS idx_services_company ON public.services USING btree (company_id);
...
CREATE INDEX IF NOT EXISTS idx_services_organization ON public.services USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_session_user_id ON public.session USING btree ("userId");
...
CREATE INDEX IF NOT EXISTS idx_setup_templates_industry ON public.setup_templates USING btree (industry);
...
CREATE INDEX IF NOT EXISTS idx_software_contacts_contact ON public.software_contacts USING btree (contact_id);
...
CREATE INDEX IF NOT EXISTS idx_software_contacts_software ON public.software_contacts USING btree (software_id);
...
CREATE INDEX IF NOT EXISTS idx_software_org ON public.software USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_tags_org ON public.tags USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_task_time_entries_date ON public.task_time_entries USING btree (work_date);
...
CREATE INDEX IF NOT EXISTS idx_task_time_entries_task ON public.task_time_entries USING btree (task_id);
...
CREATE INDEX IF NOT EXISTS idx_task_time_entries_user ON public.task_time_entries USING btree (user_id);
...
CREATE INDEX IF NOT EXISTS idx_team_members_available ON public.team_members USING btree (is_available) WHERE (is_available = true);
...
CREATE INDEX IF NOT EXISTS idx_team_members_user ON public.team_members USING btree (user_id) WHERE (user_id IS NOT NULL);
...
CREATE INDEX IF NOT EXISTS idx_teams_active ON public.teams USING btree (is_active) WHERE (is_active = true);
...
CREATE INDEX IF NOT EXISTS idx_ticket_approvals_ticket ON public.ticket_approvals USING btree (ticket_id);
...
CREATE INDEX IF NOT EXISTS idx_ticket_assets_asset ON public.ticket_assets USING btree (asset_id);
...
CREATE INDEX IF NOT EXISTS idx_ticket_assets_ticket ON public.ticket_assets USING btree (ticket_id);
...
CREATE INDEX IF NOT EXISTS idx_ticket_categories_active ON public.ticket_categories USING btree (is_active) WHERE (is_active = true);
...
CREATE INDEX IF NOT EXISTS idx_ticket_history_ticket ON public.ticket_history USING btree (ticket_id);
...
CREATE INDEX IF NOT EXISTS idx_ticket_replies_ticket ON public.ticket_replies USING btree (ticket_id);
...
CREATE INDEX IF NOT EXISTS idx_ticket_status_history_created ON public.ticket_status_history USING btree (created_at);
...
CREATE INDEX IF NOT EXISTS idx_ticket_status_history_ticket ON public.ticket_status_history USING btree (ticket_id);
...
CREATE INDEX IF NOT EXISTS idx_ticket_status_transitions_from ON public.ticket_status_transitions USING btree (from_status_id);
...
CREATE INDEX IF NOT EXISTS idx_ticket_status_transitions_org ON public.ticket_status_transitions USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_ticket_statuses_base ON public.ticket_statuses USING btree (base_status);
...
CREATE INDEX IF NOT EXISTS idx_ticket_statuses_enabled ON public.ticket_statuses USING btree (is_enabled) WHERE (is_enabled = true);
...
CREATE INDEX IF NOT EXISTS idx_ticket_statuses_org ON public.ticket_statuses USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_ticket_tasks_assigned ON public.ticket_tasks USING btree (assigned_to) WHERE ((assigned_to IS NOT NULL) AND (is_completed = false));
...
CREATE INDEX IF NOT EXISTS idx_ticket_tasks_service_cat ON public.ticket_tasks USING btree (service_category) WHERE ((service_category IS NOT NULL) AND (is_completed = false));
...
CREATE INDEX IF NOT EXISTS idx_ticket_tasks_ticket ON public.ticket_tasks USING btree (ticket_id);
...
CREATE INDEX IF NOT EXISTS idx_ticket_time_entries_ticket ON public.ticket_time_entries USING btree (ticket_id);
...
CREATE INDEX IF NOT EXISTS idx_ticket_time_entries_user ON public.ticket_time_entries USING btree (user_id);
...
CREATE INDEX IF NOT EXISTS idx_ticket_types_org ON public.ticket_types USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_tickets_assigned_status ON public.tickets USING btree (assigned_to, status_id) WHERE (assigned_to IS NOT NULL);
...
CREATE INDEX IF NOT EXISTS idx_tickets_category ON public.tickets USING btree (request_category);
...
CREATE INDEX IF NOT EXISTS idx_tickets_contact ON public.tickets USING btree (contact_id);
...
CREATE INDEX IF NOT EXISTS idx_tickets_created ON public.tickets USING btree (created_at);
...
CREATE INDEX IF NOT EXISTS idx_tickets_deleted ON public.tickets USING btree (is_deleted) WHERE (is_deleted = true);
...
CREATE INDEX IF NOT EXISTS idx_tickets_first_viewed ON public.tickets USING btree (first_viewed_at) WHERE (first_viewed_at IS NULL);
...
CREATE INDEX IF NOT EXISTS idx_tickets_number ON public.tickets USING btree (organization_id, ticket_number);
...
CREATE INDEX IF NOT EXISTS idx_tickets_organization ON public.tickets USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_tickets_embedding_status ON public.tickets (embedding_status) WHERE embedding_status != 'complete';
...
CREATE INDEX IF NOT EXISTS idx_tickets_resolution_category ON public.tickets USING btree (organization_id, resolution_category) WHERE (resolution_category IS NOT NULL);
...
CREATE INDEX IF NOT EXISTS idx_tickets_sla_due ON public.tickets USING btree (sla_due_at) WHERE (sla_breached = false);
...
CREATE INDEX IF NOT EXISTS idx_tickets_subcategory ON public.tickets USING btree (subcategory_id);
...
CREATE INDEX IF NOT EXISTS idx_tickets_team ON public.tickets USING btree (team_id);
...
CREATE INDEX IF NOT EXISTS idx_tickets_unassigned_open ON public.tickets USING btree (organization_id, created_at DESC) WHERE (assigned_to IS NULL);
...
CREATE INDEX IF NOT EXISTS idx_tickets_workspace ON public.tickets USING btree (workspace_id);
...
CREATE INDEX IF NOT EXISTS idx_user_assignment_history_assignment ON public.user_assignment_history USING btree (assignment_id);
...
CREATE INDEX IF NOT EXISTS idx_user_assignments_organization ON public.user_assignments USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_user_assignments_resource ON public.user_assignments USING btree (resource_type, resource_id);
...
CREATE INDEX IF NOT EXISTS idx_user_assignments_status ON public.user_assignments USING btree (status);
...
CREATE INDEX IF NOT EXISTS idx_user_assignments_user ON public.user_assignments USING btree (user_id);
...
CREATE INDEX IF NOT EXISTS idx_user_group_memberships_group ON public.user_group_memberships USING btree (group_id);
...
CREATE INDEX IF NOT EXISTS idx_user_group_memberships_user ON public.user_group_memberships USING btree (user_id);
...
CREATE INDEX IF NOT EXISTS idx_user_groups_external ON public.user_groups USING btree (external_source, external_id);
...
CREATE INDEX IF NOT EXISTS idx_user_groups_org ON public.user_groups USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_user_groups_owner ON public.user_groups USING btree (owner_id);
...
CREATE INDEX IF NOT EXISTS idx_user_groups_parent ON public.user_groups USING btree (parent_id);
...
CREATE INDEX IF NOT EXISTS idx_user_groups_path ON public.user_groups USING btree (path);
...
CREATE INDEX IF NOT EXISTS idx_user_groups_type ON public.user_groups USING btree (group_type);
...
CREATE INDEX IF NOT EXISTS idx_user_help_dismissals_user ON public.user_help_dismissals USING btree (user_id);
...
CREATE INDEX IF NOT EXISTS idx_user_notes_user ON public.user_notes USING btree (user_id, is_archived);
...
CREATE INDEX IF NOT EXISTS idx_user_service_access_active ON public.user_service_access USING btree (contact_id, service_id) WHERE (revoked_at IS NULL);
...
CREATE INDEX IF NOT EXISTS idx_user_service_access_contact ON public.user_service_access USING btree (contact_id);
...
CREATE INDEX IF NOT EXISTS idx_user_service_access_service ON public.user_service_access USING btree (service_id);
...
CREATE INDEX IF NOT EXISTS idx_user_tasks_user ON public.user_tasks USING btree (user_id, is_archived);
...
CREATE INDEX IF NOT EXISTS idx_users_external ON public.users USING btree (external_source, external_id) WHERE (external_id IS NOT NULL);
...
CREATE INDEX IF NOT EXISTS idx_users_organization ON public.users USING btree (organization_id);
...
CREATE INDEX IF NOT EXISTS idx_users_contact ON public.users USING btree (contact_id);

--
-- Name: idx_workspace_members_user; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX IF NOT EXISTS idx_workspace_members_user ON public.workspace_members USING btree (user_id) WHERE (user_id IS NOT NULL);
...
CREATE INDEX IF NOT EXISTS idx_workspace_members_workspace ON public.workspace_members USING btree (workspace_id);
...
CREATE INDEX IF NOT EXISTS idx_workspaces_active ON public.workspaces USING btree (is_active) WHERE (is_active = true);
...
CREATE INDEX IF NOT EXISTS idx_workspaces_org ON public.workspaces USING btree (organization_id);
...
-- Backward-compatibility view for migration 053
--

CREATE OR REPLACE VIEW public.clients AS SELECT * FROM public.companies;
...
DROP TRIGGER IF EXISTS department_path_trigger ON public.departments;
...
DROP TRIGGER IF EXISTS location_path_trigger ON public.office_locations;
...
DROP TRIGGER IF EXISTS ticket_status_change_trigger ON public.tickets;
...
DROP TRIGGER IF EXISTS trigger_generate_task_number ON public.project_tasks;
...
DROP TRIGGER IF EXISTS trigger_set_domain_history_severity ON public.domain_history;
...
DROP TRIGGER IF EXISTS trigger_task_update_project_progress ON public.project_tasks;
...
DROP TRIGGER IF EXISTS trigger_update_accessory_quantities ON public.accessory_inventory_transactions;
...
DROP TRIGGER IF EXISTS trigger_update_group_path ON public.user_groups;
...
ALTER TABLE ONLY public.access_requests
    ADD CONSTRAINT access_requests_for_contact_id_fkey FOREIGN KEY (for_contact_id) REFERENCES public.contacts(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY public.access_requests
    ADD CONSTRAINT access_requests_requester_id_fkey FOREIGN KEY (requester_id) REFERENCES public.contacts(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY public.asset_contacts
    ADD CONSTRAINT asset_contacts_contact_id_fkey FOREIGN KEY (contact_id) REFERENCES public.contacts(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY public.asset_requests
    ADD CONSTRAINT asset_requests_for_contact_id_fkey FOREIGN KEY (for_contact_id) REFERENCES public.contacts(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY public.asset_requests
    ADD CONSTRAINT asset_requests_requester_id_fkey FOREIGN KEY (requester_id) REFERENCES public.contacts(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY public.bulk_access_operations
    ADD CONSTRAINT bulk_access_operations_target_contact_id_fkey FOREIGN KEY (target_contact_id) REFERENCES public.contacts(id) ON DELETE CASCADE;
...
--
-- Name: client_tag_assignments company_tag_assignments_company_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: client_tags client_tags_organization_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
ALTER TABLE ONLY public.contact_credentials
    ADD CONSTRAINT contact_credentials_contact_id_fkey FOREIGN KEY (contact_id) REFERENCES public.contacts(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY public.dynamic_group_members
    ADD CONSTRAINT dynamic_group_members_contact_id_fkey FOREIGN KEY (contact_id) REFERENCES public.contacts(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY public.email_preferences
    ADD CONSTRAINT email_preferences_contact_id_fkey FOREIGN KEY (contact_id) REFERENCES public.contacts(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY public.kb_contributors
    ADD CONSTRAINT kb_contributors_contact_id_fkey FOREIGN KEY (contact_id) REFERENCES public.contacts(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY public.kb_permissions
    ADD CONSTRAINT kb_permissions_contact_id_fkey FOREIGN KEY (contact_id) REFERENCES public.contacts(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY public.maintenance_schedules
    ADD CONSTRAINT maintenance_schedules_location_id_fkey FOREIGN KEY (location_id) REFERENCES public.locations(id);
...
ALTER TABLE ONLY public.offboarding_requests
    ADD CONSTRAINT offboarding_requests_contact_id_fkey FOREIGN KEY (contact_id) REFERENCES public.contacts(id);
...
ALTER TABLE ONLY public.onboarding_requests
    ADD CONSTRAINT onboarding_requests_contact_id_fkey FOREIGN KEY (contact_id) REFERENCES public.contacts(id);
...
ALTER TABLE ONLY public.onboarding_requests
    ADD CONSTRAINT onboarding_requests_job_title_id_fkey FOREIGN KEY (job_title_id) REFERENCES public.job_titles(id);
...
ALTER TABLE ONLY public.service_owners
    ADD CONSTRAINT service_owners_contact_id_fkey FOREIGN KEY (contact_id) REFERENCES public.contacts(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY public.software_contacts
    ADD CONSTRAINT software_contacts_contact_id_fkey FOREIGN KEY (contact_id) REFERENCES public.contacts(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY public.team_members
    ADD CONSTRAINT team_members_contact_id_fkey FOREIGN KEY (contact_id) REFERENCES public.contacts(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY public.user_service_access
    ADD CONSTRAINT user_service_access_contact_id_fkey FOREIGN KEY (contact_id) REFERENCES public.contacts(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY public.workspace_members
    ADD CONSTRAINT workspace_members_contact_id_fkey FOREIGN KEY (contact_id) REFERENCES public.contacts(id) ON DELETE CASCADE;
...
-- ============================================================================
-- SEED DATA: Reference tables that must be populated for the app to function
-- ============================================================================

-- Ticket status templates (used by initialize_ticket_statuses() during setup wizard)
